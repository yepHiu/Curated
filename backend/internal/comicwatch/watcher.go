// Package comicwatch watches configured comic library roots with fsnotify and
// debounces scan requests when zip/cbz archives appear or change.
package comicwatch
...
// PathLister returns current comic library root paths.
type PathLister interface {
	ListComicLibraryPathStrings(ctx context.Context) ([]string, error)
}

// ScanQueue receives merged comic roots to scan after debouncing.
type ScanQueue interface {
	EnqueueComicLibraryWatchScanRoots(roots []string)
}
...
// Watcher watches comic library directories for new/changed archives.
type Watcher struct {
	opts Options
...
var comicArchiveExtensions = map[string]struct{}{
	"zip": {},
	"cbz": {},
}

// New builds a watcher. It does not start I/O until Run.
...
func (w *Watcher) Run(ctx context.Context) error {
	...
	for {
		...
		if err := w.bootstrap(ctx); err != nil {
			w.opts.Logger.Warn("comic watch bootstrap failed, retrying", zap.Error(err))
			select {
			case <-ctx.Done():
				return ctx.Err()
...
func (w *Watcher) bootstrap(ctx context.Context) error {
	...
	roots, err := w.opts.Lister.ListComicLibraryPathStrings(ctx)
	if err != nil {
		return err
	}
	roots = cleanRoots(roots)
	if len(roots) == 0 {
		w.opts.Logger.Debug("comic watch: no comic library paths configured")
	}
	...
	for _, root := range roots {
		if st, err := os.Stat(root); err != nil || !st.IsDir() {
			w.opts.Logger.Debug("comic watch: skip missing or non-dir root", zap.String("root", root), zap.Error(err))
			continue
		}
		_ = filepath.WalkDir(root, func(path string, d os.DirEntry, walkErr error) error {
			...
			if err := w.addWatchLocked(path); err != nil {
				w.opts.Logger.Debug("comic watch: add failed", zap.String("path", path), zap.Error(err))
			}
			return nil
		})
	}
	w.opts.Logger.Info("comic fsnotify watching", zap.Int("dirCount", len(w.watched)))
	return nil
}
...
func (w *Watcher) consumeEvents(ctx context.Context, fsn *fsnotify.Watcher) (channelClosed bool) {
	for {
		...
		case err, ok := <-fsn.Errors:
			...
			if err != nil {
				w.opts.Logger.Debug("comic watch fsnotify error", zap.Error(err))
			}
		...
		if w.watcher != nil {
			_ = filepath.WalkDir(path, func(p string, d os.DirEntry, walkErr error) error {
				...
				if err := w.addWatchLocked(p); err != nil {
					w.opts.Logger.Debug("comic watch: add subtree", zap.String("path", p), zap.Error(err))
				}
				return nil
			})
		}
...
func (w *Watcher) scheduleScanForPath(ctx context.Context, filePath string) {
	roots, err := w.opts.Lister.ListComicLibraryPathStrings(ctx)
	if err != nil {
		w.opts.Logger.Warn("comic watch: list roots failed", zap.Error(err))
		return
	}
	roots = cleanRoots(roots)
...
func (w *Watcher) flushDebounce() {
	...
	slices.Sort(roots)
	w.opts.Logger.Info("comic watch: enqueue scan", zap.Strings("roots", roots))
	w.opts.Queue.EnqueueComicLibraryWatchScanRoots(roots)
}
...
func isComicArchivePath(p string) bool {
	...
	_, ok := comicArchiveExtensions[ext]
	return ok
}
