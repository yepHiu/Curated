...
// TestHandlePatchSettings_PhotoBetaCanEnableBeforeAddingPath covers the enable-first Beta flow.
func TestHandlePatchSettings_PhotoBetaCanEnableBeforeAddingPath(t *testing.T) {
	t.Parallel()
	...
	if !ctl.enabled {
		t.Fatal("photo beta should enable before configuring paths")
	}
}
