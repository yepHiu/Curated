...
export async function probeServer(input: string, signal?: AbortSignal, fetchImpl = fetch): Promise<ServerInfo> {
  ...
  if (response.status === 404 || response.status === 423) {
    ...
    if (response.status === 423) {
      const version = "version" in payload ? String(payload.version) : ""
      if (!/^1\.[0-6]\.\d+$/.test(version)) throw new Error("服务器身份接口被锁定，请检查 Server 版本。")
    }
...
// A legacy endpoint gains its first UUID during a normal Server upgrade.
export function hasServerIdentityChanged(previous: string | undefined, next: string, url: string): boolean {
  return !!previous && previous !== `legacy:${normalizeServerUrl(url)}` && previous !== next
}
