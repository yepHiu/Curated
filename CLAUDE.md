...
- **`lanEnabled`** - Whether the HTTP server should bind for LAN clients (default: `false`); Settings → Network persists this without requiring a PIN, and a full quit/reopen rebinds `0.0.0.0` on the same port. Host/CORS still follow the current listener until restart.
...
- REST API at `/api`
- Frontend connects via HTTP when `VITE_USE_WEB_API=true`; backend defaults are loopback-only (`127.0.0.1:8080` dev, `127.0.0.1:8081` release). Settings → Network can persist `lanEnabled: true` without a PIN; the next full restart binds `0.0.0.0` on the same port. A non-loopback main-config `httpAddr` also requires `lanEnabled: true`.
