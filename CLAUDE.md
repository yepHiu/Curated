...
**Windows CD:** `.github/workflows/cd-release.yml` validates version-tag source/notes, calls CI on the exact commit, and builds the legacy Windows x64 installer/ZIP. Tag pushes publish; manual existing-tag runs default to draft. `cd_release.py` verifies package and GitHub upload digests, resumes only same-source drafts, and refuses published-version overwrites. The workflow uses the built-in token with write access only in its upload job. First Windows runner execution remains to be verified; see `docs/guide.md` §8. No API changes or independent component packages are introduced.
...
**Server version:** Health (HTTP and stdio) now separates numeric `version` (1.5.7) from UTC `buildStamp` and `channel`. Go embeds the canonical `backend/internal/version/server.json`; the release planner reads the same file. About shows both version and timestamp. Explicit BuildStamp injection is compilation time; the existing Git-time fallback is commit time. Legacy installer version/update comparison remains separate.
