...
- Settings -> About now includes packaged-app update status, a manual update-check action, in-app latest `.exe` installer download with SHA256 verification, explicit installer launch when ready, and a release-page fallback link; when an update is available, the sidebar shows a lightweight `New` badge (expanded) or dot (compact) that links to `Settings -> About`, while the `Curated` brand text/icon links to the home page
