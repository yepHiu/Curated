...
**Desktop version bridge:** `window.javLibrary.getDesktopInfo()` returns local build metadata (Desktop 0.1.0, development flag, distribution, platform and arch); `checkDesktopUpdate()` performs local manifest checking. Both IPC handlers validate the current main window/frame/origin. No new HTTP endpoint. Development builds never request production updates; current legacy bundles still update via the old installer. The About page hides Desktop in browsers. Component version sources and read-only `plan-component` naming are implemented; standalone packaging/migration and automatic Desktop installation remain pending. See plan §11.5 and the guide.
