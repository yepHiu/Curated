...
**Current Architecture Phase:** Local-first Electron delivery around a shared Vue SPA + Go HTTP API boundary. Electron currently starts or reuses the Go backend, starts or reuses Vite in development, uses the Curated app icon, hides to tray on window close, marks backend requests as Curated Desktop with `X-Curated-Client: desktop-electron` plus desktop OS headers, versions its renderer entry URL across app upgrades, and exposes a narrow `window.javLibrary.pickDirectory()` preload bridge for native directory selection plus read-only `windowChrome` for the macOS integrated titlebar. Production release packaging installs `Curated.exe` as the Electron desktop shell, bundles the Go backend under `resources/app/curated.exe`, serves entry HTML without caching while keeping hashed assets immutable, and cleans only managed `frontend-dist` directories before installer upgrades; deeper Electron IPC bridges and mpv player integration remain target-direction work.

**CD:** `.github/workflows/cd-release.yml` checks the exact tag commit and builds Windows legacy packages plus Apple Silicon standalone Mac Desktop DMG/ZIP. Both platforms must succeed; versions, release notes and uploaded digests are validated before publication. `package-macos-desktop` uses independent Desktop metadata, bundles no Server/FFmpeg, and ad-hoc signs the app without Apple notarization. Tag pushes publish; manual existing-tag runs default to draft. Published releases cannot be overwritten. Mac packaging and startup were validated locally; the new Mac job awaits a subsequent release tag. See guide §8.
...
- **`autoDownloadUpdates`** - Whether startup background update checks may automatically download and SHA256-verify a newer installer after one is discovered (default: `false`); exposed in Settings -> About & updates, and installation still remains an explicit user action
...
- **`lanEnabled`** - Whether the HTTP server should bind for LAN clients (default: `false`); Settings → Network & devices persists this without requiring a PIN, and a full quit/reopen rebinds `0.0.0.0` on the same port. Host/CORS still follow the current listener until restart.
...
```
GET    /api/health                          # Health check (name, numeric Server version, buildStamp, optional installerVersion, channel, databasePath)
...
PATCH  /api/auth/settings                   # Update non-secret PIN lock settings; pinEnabled=false disables PIN independently of LAN access
...
POST   /api/app-update/download             # Direct local only: download and SHA256-verify installer
POST   /api/app-update/install              # Direct local only: launch verified installer after user action
DELETE /api/app-update/downloaded-installer # Direct local only: clear installer metadata/file
...
**Agent answer publication (2026-09-09):** Chat uses request-scoped `answerRefs` and read-only `submit_answer` to fill movie facts on the server. Raw reasoning is not sent; `answer_progress` reports preparation, checked content uses `text_delta`, and `message_done.answerEvidence` optionally stores version-1 per-record source snapshots in existing `events_json`. A 15-second SSE heartbeat is independent of each model call's two-minute deadline. Invalid movie drafts receive at most one correction and then `partial` / `answer_rejected`. `present_movies` requires a local read and ignores free-text reasons. No new HTTP endpoint or configuration field. Historical refs grant no authority; standalone Actions and source-data correctness have separate limits.
...
**Library path management:** HTTP health exposes request-local `canManageLibraryPaths` (no-store). Three-library root POST/PATCH/DELETE, movie reveal/rebind, and PATCH settings default import ids require direct loopback access; remote path writes return `403 LIBRARY_PATHS_READ_ONLY` (settings PATCH uses `SERVER_SETTINGS_READ_ONLY`). Mixed settings requests are rejected before mutations. Frontend checks the API and Desktop main-owned Server origin and shows remote/unknown paths read-only; uploads still use Server destinations.

**Server settings administration:** `PATCH /api/settings` (including default import ids) and PIN/session administration, connected clients, backups/health maintenance, comic cache cleanup, provider/proxy tests and AI administration require direct local access; remote requests return `403 SERVER_SETTINGS_READ_ONLY`. Remote GET settings removes AI key/Base URL and proxy URL/credentials and is no-store. UI hides these controls and inaccessible deep links, preserving client preferences, current-session unlock/lock, Desktop connections and ordinary business APIs. See `docs/guide.md` → Remote settings access.
...
**Server version:** Health (HTTP and stdio) now separates numeric `version` (1.5.8) from UTC `buildStamp` and `channel`. Go embeds the canonical `backend/internal/version/server.json`; the release planner reads the same file. About shows both version and timestamp. Explicit BuildStamp injection is compilation time; the existing Git-time fallback is commit time. Legacy installer version/update comparison remains separate.

**Desktop version bridge:** `window.javLibrary.getDesktopInfo()` returns local build metadata (Desktop 0.1.0, development flag, distribution, platform, arch and actual `serverOrigin`); `checkDesktopUpdate()` performs local manifest checking. Both IPC handlers validate the current main window/frame/origin. No new HTTP endpoint. Development builds never request production updates; current legacy bundles still update via the old installer. The About page hides Desktop in browsers. Component version sources and read-only `plan-component` naming are implemented; standalone packaging/migration and automatic Desktop installation remain pending. See plan §11.5 and the guide.
...
**App update checks/download/install:** HTTP responses include request-local `localUpdateAllowed` (no-store); direct loopback peer/Host/Origin with no proxy headers is required for download/install/cache-delete, otherwise 403 `APP_UPDATE_REMOTE_DISABLED`. Frontend also verifies the API target and Desktop main-owned Server origin; standalone Desktop or unknown/remote connections only expose independent Desktop downloads, with no Full fallback. Automatic client downloads obey the same gate. Reverse proxies must preserve external Host or forwarding information. `GET /api/app-update/status` returns the packaged-app update state used by Settings -> About & updates and the sidebar brand badge, while `POST /api/app-update/check` forces a refresh. The backend compares the current runtime `installerVersion` with the latest GitHub Release for `yepHiu/Curated`, returns `installerDownloadUrl` and `installerSha256` when the release includes a Windows `.exe` installer asset with a digest, caches the result in SQLite, reuses the process proxy settings for outbound requests, and uses `0.0.0` as the dev-runtime fallback when no packaged installer version was injected. Settings -> About & updates exposes persisted `autoDownloadUpdates`; when enabled, the startup background check may automatically download and SHA256-verify a newer installer, but it never auto-installs one. `POST /api/app-update/download` downloads to the backend update cache and verifies SHA256; `POST /api/app-update/install` launches the verified installer only after explicit user action. Current local packages are unsigned, so this flow relies on SHA256 integrity and keeps silent auto-install disabled by default.
...
- REST API at `/api`
- Frontend connects via HTTP when `VITE_USE_WEB_API=true`; backend defaults are loopback-only (`127.0.0.1:8080` dev, `127.0.0.1:8081` release). Settings → Network & devices can persist `lanEnabled: true` without a PIN; the next full restart binds `0.0.0.0` on the same port. A non-loopback main-config `httpAddr` also requires `lanEnabled: true`.
- Electron shell MVP under `electron/`: for the default local legacy connection starts or reuses the Go HTTP backend, waits for `/api/health`, starts or reuses Vite at `http://127.0.0.1:5173` in development, loads the Web UI in BrowserWindow with the Curated app icon and a versioned renderer query, keeps the app running in the tray when the window is closed, and exposes `window.javLibrary.pickDirectory()` through preload for native directory selection and read-only `windowChrome` for platform window styling. Packaged releases install `Curated.exe` as the Electron shell, package Electron app files under `resources/app/electron-dist`, place the Go backend at `resources/app/curated.exe`, load the backend-hosted static UI on `http://127.0.0.1:8081`, serve entry HTML with `no-store`, and remove managed old `frontend-dist` trees before installer upgrades.
...
- Settings -> About & updates now includes packaged-app update status, a manual update-check action, in-app latest `.exe` installer download with SHA256 verification, explicit installer launch when ready, and a release-page fallback link; Settings -> About & updates also provides persisted `autoDownloadUpdates` for opt-in startup background download-and-verify behavior; when an update is available, the sidebar shows a lightweight `New` badge (expanded) or dot (compact) that links to `Settings -> About & updates`, while the `Curated` brand text/icon links to the home page
...
Photo per-book progress/preferences APIs and concrete cache cleanup are not implemented. `photoCache.maxBytes` remains reserved. The default photo import target is used by the Add media photo Tab. Closing Beta preserves settings, indexes and archives and stops its watcher. Settings → Libraries → Photo library keeps the Beta switch visible; path, viewer, watcher and cache controls appear only while the library is enabled.

### Agent domain boundary

Agent context continuity (2026-09-21): `POST /api/ai/chat` additionally emits `context_status` with `context.phase=compacting|ready|limited`, under one `message_start` and monotonic sequence. Migration 0052 stores independent session checkpoints; original messages stay intact, session deletion cascades to checkpoints. Recent history is combined with incrementally summarized older goals; same-session chat requests are serialized. Settings GET/PATCH persists aiProvider.contextWindow (default 65536 tokens, integers 32768–2097152); model presets fill capacity only. Each chat snapshots this setting. Working context reserves min(32768, W/4) output and W/20 safety, then compacts at 80% of the remaining input budget, conservatively charging JSON bytes rather than measured tokens; explicit provider context overflow has one recovery retry. Irreducible input returns `context_input_too_large` and can continue in the same session. `agent-system-v7` treats summaries as untrusted history without granting fact references or write authority. No new endpoint; contextWindow is the new library-config field. Chat and summary requests send the output reserve as max_tokens; standalone Actions and connectivity probes keep their existing policy. See `docs/plan/2026-09-21-agent-context-continuity.md` for limits and deferred run recovery.
...
- `GET /api/wishlist/items/{id}/playback`: on-demand fixed-site availability check (Jable and MISSAV), returning available/missing/blocked/unknown. Blocked responses (403/429/Cloudflare challenge) include the fixed detail URL for manual verification without claiming availability; protected application session.
...
### Desktop connection IPC (2026-09-26)

`window.javLibrary.getServerConnections()` returns a read-only names/addresses/current-origin snapshot to the current trusted renderer. `openServerConnections(savedId?)` opens the bundled, offline-capable manager, optionally requesting an existing saved target through native switch confirmation. `addServer({name,url})` permits only the current trusted main-window main frame to add a new record, rejecting supplied IDs and duplicate origins. Settings → Network & devices uses an in-page Dialog to add a server, then refreshes the list without switching; management remains in the bundled window. The separate `curated:connections` IPC supports list/save/remove/connect only from the exact local manager window main frame. `electron/server-connections.ts` persists named HTTP/HTTPS origins and last success in userData/servers.json, validates health identity and partitions cookies/storage by origin including port. No HTTP endpoint or library-config field was added. Target loading is completed before replacing the current renderer; failed remote connection never starts a local Server. The default local legacy target retains owned backend/Vite lifecycle. See guide: Desktop server connections.
