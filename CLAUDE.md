...
- Settings -> About now includes packaged-app update status, a manual update-check action, and a direct latest `.exe` installer download link when GitHub Release assets expose one, falling back to the release-page link; when an update is available, the sidebar shows a lightweight `New` badge (expanded) or dot (compact) that links to `Settings -> About`, while the `Curated` brand text/icon links to the home page
