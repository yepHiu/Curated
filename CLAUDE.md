...
**CD:** `cd-release.yml` builds Windows Full/Server/Desktop and Apple Silicon Desktop, using component tags, independent versions and exact component manifests. Full embeds the standalone installers; Windows installation/lifecycle smoke checks gate publication. `cd-legacy.yml` retains compatible legacy v-tag releases. New stable releases do not replace the old `/releases/latest` feed; see guide §8 for isolation, migration and recovery.
