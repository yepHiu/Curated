...
- Web API モードでは、ホームの hero と「今日のおすすめ」に UTC 日次で切り替わる SQLite 永続スナップショットを使い、ブラウザや端末をまたいでも同じ結果を表示しつつ、作品ごとの推薦状態、ハードクールダウン、重み付きサンプリング、推薦回数による減衰、出演者とスタジオの偏り抑制で、連日または隔週の重複や同日の偏在をできるだけ減らします。
- Windows リリースではトレイモード起動、ローカル Web 配信、インストーラーパッケージ生成をサポート。
- 現在の Web フェーズでも、俳優メタデータ、キュレートフレームのエクスポート、再生セッション診断を利用可能。
- 現在の Web フェーズでは、DualSense を含む標準 Gamepad API コントローラーによるグローバルフォーカス移動、ライブラリグリッド選択、プレイヤー操作、設定画面のローカルトグルをサポート。
...
- 仮想化ポスターグリッドは標準ゲームパッド操作に対応し、DOM に全カードが描画されていない大規模リストでも URL の選択状態を使って移動します。
...
- 標準ゲームパッドで再生 / 一時停止、シーク、音量、ミュート、キュレートフレーム保存、統計 / 操作レイヤー切り替え、全画面解除、戻る操作を実行できます。
