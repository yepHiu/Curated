...
### 15.3 AI 设置自动保存（2026-09-06）

用户要求取消额外保存动作。治理策略与模型配置均改为自动保存：开关/策略选项立即提交，数字/文本停顿 550ms 或失焦后提交。两个配置组分别串行写入，加载初始值不触发保存；响应只确认已提交快照，不覆盖后续输入。无效数字不提交并显示范围提示；失败在对应卡片保留草稿并提供重试。切换分区时提交待保存的有效输入。

两个保存按钮已移除，固定的卡片内状态行显示自动保存、保存中、已保存及失败。成功收到服务端治理配置后才更新全局权限；策略变更停止生成的既有行为保留并在三语提示中明确。连通测试和清理仍需点击；自动保存不刷新统计/审计，也不改分页位置。Web/Mock 使用既有服务合约，无新增 API。

验证：19 项 AI 设置组件测试与 8 项三语一致性测试通过，覆盖初始加载不写回、数值防抖、立即开关、串行写入、旧响应/Provider 乐观回滚不覆盖新草稿、失败重试、失焦/切换分区提交以及连通测试前保存。类型检查、改动代码的 ESLint 与 `git diff --check` 通过。隔离 Mock 页面实测只读模式、步骤上限、模型地址和名称自动保存，刷新后值保留；无效数字显示范围提示。375px 视口无横向溢出，两个保存按钮均已移除。未运行 display-scaling 套件，未调用真实 Provider；验证用临时页面和 Vite 进程已关闭。

## 16. 番号事实约束（2026-09-09，首版已实施；原建议与细化计划保留）
...
- Provider 工具已有库内/库外标记与本地对账。库外条目没有本地 movieId，不能为加强约束而简单删除所有库外结果。

因此可确认的缺口是自由文本绕过了已有实体引用约束；不能据此断言某次错误一定由这个路径产生。

### 16.2 目标与可信度定义

目标是让作为作品事实展示的番号可追溯到本轮服务端取得的具体记录，并保持番号、标题、演员与同一条记录的对应关系。仅验证番号存在，仍可能把真实番号配给错误作品。

区分本地记录、外部来源记录、用户输入的待核实查询。用户输入、模型记忆、旧 assistant 正文和歧义候选均不能自动成为已验证事实。本地查无结果只能说明当前资料库未查到；外部来源记录也可能错误，系统保证的是引用一致性与可追溯性，不能承诺全世界元数据绝对正确。

### 16.3 建议实施顺序

1. **收紧提示词与评测基线。** 显式要求番号必须来自工具记录，禁止按系列前缀或数字规律补全；查不到时说明证据范围。模型参数调整只能辅助降低发生率，不作为验收保证。
2. **建立本轮事实引用表并改为结构化输出。** 复用 MovieRefStore，为外部结构化条目补充独立 sourceRef，保存规范番号、原始番号、标题、来源、检索时间及库内状态。模型选择服务端签发的引用；服务端验证引用后填入番号/标题。仅有页面 ID 的引用需要读取资料后才能展示具体番号。来源页面里的任意文本不能自动全部授信，必须建立到具体作品记录的可核对映射。
3. **在输出前执行确定性校验。** 对正文和卡片 reason 中出现的番号核对事实引用；最好让事实字段使用引用片段，减少自由文本重复番号。检测未知或错配引用时，最多允许一次有界修正；仍失败则用服务端模板说明未能核实并保留有效结果，以 partial/needs_input 等既有状态收口。不能只是加一个“仅供参考”标记后继续展示虚构推荐。
4. **统一流式与持久化边界。** 首个可靠版本可缓冲每次模型调用的全部用户可见正文，校验通过后再发布；工具过程事件仍可即时显示。后续若优化成分段校验，需覆盖跨 delta、跨段番号及 Markdown 格式拆分。不能先流出再事后撤回。thinking_delta 若继续包含原始模型文字也会绕过屏障，严格模式应改为服务端工具进度摘要。历史只保存发布后的正文；旧 assistant 内容不能自动加入本轮可信引用表。

番号检测复用 `backend/internal/library/moviecode` 的现有规范化能力，并确认其实际支持范围；大小写、连字符、全角、数字型及特殊格式需要明确处理。正则只能识别疑似番号，不能证明真实存在，也不能独立保证任意自由文本没有虚构作品。用户提问中复述的未核实番号应通过受控的“查询对象”展示，与推荐结果分开。

### 16.4 验收与边界

- 无工具证据、检索失败或空结果时，模型捏造的作品不能进入已发布正文或卡片理由。
- 本轮命中记录后，合法引用可显示；真实番号配错标题/演员的结构化输出必须拒绝。
- 外部来源命中但本地没有时，可显示来源与未入库状态，不创建本地卡片或本地 ID。
- 歧义候选、旧回答、用户输入不得直接升级为已确认推荐。
- 覆盖跨流式 chunk、Markdown/全角形式、取消、失败、步数用尽、修正预算用尽、历史重开和并发请求引用隔离。
- 先用 scripted streamer 做故意输出错误番号的确定性测试，再用非敏感合成数据做真实模型回归；降低温度或更换模型不能替代这些测试。
- “未查到”不得扩展成“该作品不存在”；演员和评价等自由文本事实仍需要字段级证据，不能将番号校验通过宣传为整段回答全部正确。

建议先完成“事实引用表 + 结构化事实展示 + 展示前校验”的最小闭环，再优化首字延迟和更丰富的来源检索。本节只更新计划文档，不代表上述能力已上线。

### 16.5 具体实施计划（2026-09-09，用户要求细化，尚未实施）

**交付目标**：具体作品的推荐、列表和事实介绍通过结构化作品引用发布；番号、标题、演员、片长等标识与事实字段由服务端从同一记录填入。发布检查发生在 SSE 输出和历史保存之前。普通解释性文字可以保留，但不能把对任意自然语言的正则检查宣传为完整事实证明。

#### A. 建立按请求隔离的事实记录

- 新建 `backend/internal/agent/core/answer_refs.go`，定义 `AnswerRef` / `AnswerRefStore`。记录包含 `refId`、`runId`、可选 `movieId`、来源类型、来源条目身份、原始/规范番号、可用事实字段、检索时间、证据状态。字段应区分“未知”与有效的零值。
- 生命周期为一次请求，优先由 Loop 持有、通过请求上下文供工具访问。不要沿用只有 sessionID 的全局分桶作为新引用的隔离保证：同一会话并发请求也必须相互隔离。旧 MovieRefStore 的权限锚点继续保留；新发布层不能把旧锚点自动升级为事实快照。
- 只对明确允许的只读工具结果做类型化提取，支持本地查询/详情/观看记录、唯一实体匹配、Provider 结构化结果。不能通用遍历任何带 id/code 的 JSON，更不能把 present_movies 的模型 reason 或写工具参数重新吸收为事实。
- 完整记录在服务端保留；给模型的投影继续遵守 auto/minimal。minimal 隐藏的标题不应为方便模型而重新泄露；模型可选择引用，由服务端向本地用户填入字段。
- 仅有页面 ID 时，经过本地读取获得事实后再签发引用；用户输入、旧 assistant 文本、歧义候选只作为查询线索。引用无效或记录字段缺失时必须显式失败/省略，不能自动模糊补齐。
- Provider 结果保留独立来源身份，不能仅凭规范番号把不同来源事实合并。`MatchSimilar` 不算已核实相同作品；遇到规范化碰撞要保留歧义。现有 moviecode 仅有有限规范化，不假设其已覆盖全角或所有特殊番号。

**本步验收**：伪造、过期、跨请求引用被拒绝；同一会话并发不串记录；minimal 投影不泄露隐藏字段；库外引用没有本地操作权限。

#### B. 增加结构化的回答提交入口

- 新增内部只读工具 `submit_answer`，沿用当前 OpenAI 兼容 tool_calls、服务端 Schema 校验与治理，不依赖某家模型的 strict JSON/response_format 特性。工具本身不查询数据库，不获取权限，只消费本轮已取得的引用。
- 建议参数采用简单对象与数组，兼容现有 Schema 子集：`items: [{ refId, fields, reasonFacts }]`；首版具体作品展示沿用最多六条。`fields` / `reasonFacts` 为受支持的字段键枚举，不能传 code/title/actors 的自定义值。普通解释性段落另设有界 text，不能承担作品事实展示。
- 服务端解析引用，检查要求的字段存在，按来源生成显示快照。推荐理由首版支持演员、片长、标签、评分、观看状态等证据字段；例如模型选择 runtime，后端从记录生成“片长……分钟”。“低于 90 分钟”之类条件判断还需要后端验证条件，不能只因为选中了 runtime 字段就生成“符合要求”。
- 来源缺失的评分/演员省略并标记未知；同一字段存在本地/来源站差异时并列来源，不能静默拼成一条更完整的作品。
- 成功提交即终结本轮，不再要求模型把结果复述一遍。提交调用与其他工具混在一批时，拒绝混合终结批次并要求先检索、后单独提交，避免提前终结和遗漏写预览。
- 旧 `present_movies` 也接入同一快照生成器；其 reason 经过发布检查，或用 reasonFacts 替代，不能保留自由理由绕过新入口。更新提示词版本及工具说明。

**本步验收**：无法向工具塞入自定义番号、标题、演员；合法引用正确呈现；无效字段被拒绝；外部来源记录不会生成可操作的本地卡片。

#### C. 设置唯一的发布关卡

- 新建 `backend/internal/agent/run/answer_guard.go` 和服务端回答渲染模块。`loop.go` 中模型原始 Content/思考回调不再直接进入用户事件。工具结果记录与可见投影分离：只读事实可显示，模型提供的摘要、推荐理由、写预览叙述不能被误当作可信工具事实直接发布。
- 结构化作品事实按 schema → 引用归属 → 字段与来源 → 渲染顺序校验。代码、标题、演员来自同一快照，避免“真实番号 + 错误作品信息”。输出为安全转义的文字/链接及卡片字段，不能将数据作为 HTML 或 Markdown 指令执行。
- 解释性自由文本先缓冲完整单次模型结果再检查。对可识别的番号、作品链接及引用占位符做规范化核对；未知番号、残缺引用、指向错误实体的链接不允许发布。具体推荐内容以结构化块为准；严格作品回答采用模板连接语，不依赖识别所有自由文本形式来承诺零虚构。
- 用户自己输入的未核实番号通过服务端“查询对象”引用表达，不能用一个任意字符串白名单让模型将其转成已确认作品。
- 第一次失败返回固定错误码与字段位置，允许最多一次修正，计入现有步数、请求预算与用量；不得重新开启无限检索。预留终结步数，低步数配置/上下文不足/超时/取消时直接使用服务端安全收尾。fallback 只陈述已取得证据，不把所有搜索命中自动说成“符合推荐条件”。
- 自由文本的语义错配无法由正则全面判定，这是明确边界。保证范围是结构化作品事实及受支持的番号形式；普通评论/解释不能贴“全部事实已验证”标签。独立翻译/润色 Action 是另一条输出链路，首版验收聚焦 Agent 聊天及其中的卡片、理由与预览叙述；不能声称所有 AI 功能都已覆盖。

**本步验收**：通过故意输出虚构番号的 scripted streamer，断言任何已发送事件与最终持久化正文都不含被拒绝的模型内容，包括终态前的事件。

#### D. 流式体验、历史与来源展示

- 用户立即看到服务端“正在查询/正在整理结果”等过程状态及已有工具进度；不再转发原始 thinking_delta。正文校验成功后通过既有 text_delta 发布，卡片仍复用现有契约。不要为模拟打字而人为增加延迟。
- `backend/internal/server/ai_handlers.go` 补约 15 秒 SSE 注释心跳并串行写响应，连接关闭时取消处理。心跳只保活，不应重置模型上游无响应期限；增加独立的上游停滞/生成期限，避免一直心跳一直等待。前端现有 reader 每次收到字节会刷新 90 秒无数据计时，要测试注释心跳确实生效。
- 如需保存作品引用依据，在 message_done 增加可选、带版本的回答证据快照字段；只包含最终发布记录及来源元数据，复用现有 events_json，不为临时引用新建数据库表。旧客户端可以忽略新增字段，正文/卡片仍可显示。
- `agent_runtime.go` 继续以发布事件累积正文，确保只保存通过校验的输出。重开历史使用当时的快照；历史引用仅用于解释来源，不授予下一轮权限，续聊具体事实需重新读取。
- 前端服务契约、Web/Mock 适配器、历史恢复、过程状态、卡片来源标记一起更新。Mock 使用显式合成记录，不能把 Mock 番号作为真实性验证样本。来源使用“本地记录/来源站点记录/尚未入库”，不显示容易误解为绝对真实的统一认证标记。

**本步验收**：慢模型不误触发客户端空闲超时，真正上游停滞可终止；取消和断流不发布未校验草稿；重开历史仍有相同正文、卡片和来源；旧消息正常显示但不被追授验证状态。

#### E. 提交顺序、测试与发布门槛

按独立行为提交：A 引用表与定向测试；B+C 结构化提交及统一发布关卡（包含必要契约/调用方，不能只启用半条链路）；D 流式与历史闭环；扩展回归及事实文档。中间未接通的能力不得描述为已完成防编造。没有用户明确指令不 push。

定向测试覆盖：未知/错配/跨轮引用、空结果仍凑数、Provider 库外记录、用户输入与歧义、全角/分隔符/特殊格式、Markdown 链接和跨 chunk、推荐理由、旧工具出口、多个终结调用、最小步数、断流/取消/并发/minimal/历史恢复。先做确定性失败注入，再用非敏感合成数据对配置模型做真实兼容性验收；真实请求记录用量，不读取实际资料库当作测试数据。

后端在 `backend/` 执行 `go test ./internal/agent/... ./internal/app/... ./internal/server/...`，集成后 `go test ./...`、`go vet ./...`；前端根目录执行相关 Vitest、`pnpm typecheck`、`pnpm lint` 和受影响的运行时 e2e，发布构建按仓库规范执行 `pnpm build`，不得放宽现有预算掩盖失败。记录合法结果误拦截、被阻止输出、修正次数与首个已发布正文延迟；治理统计不保存被拒绝原文或私人番号。

上线前同步 `.cursor/rules/project-facts.mdc`、必要的 quick reference、README 短入口、`docs/guide.md`、`CLAUDE.md` 和实现说明 HTML；若公开 SSE 契约变化，同步 `API.md`。本次仅补充计划，不执行代码变更、模型调用、服务重启或发布。

### 16.6 首版实施与验收记录（2026-09-09）

**已实现**：

- `core/answer_refs.go` 提供随机请求作用域、不可混拼的来源快照、类型限定的查询入口及模型用 hints；最多 512 条。事实在模型隐私投影前捕获，minimal 不因此泄露标题。只读锚点与步骤预算按请求隔离；审计/确认票据保留真实会话 ID。
- `core/answer_submit.go` 新增只读 `submit_answer`，接受 1–6 条 refId 和可用字段名；禁止模型自行写 code/title/actors。服务端生成三语字段标签、来源说明及卡片；Provider 结果保持独立来源，甚至已对账本地 ID 也不直接生成本地卡片。明确字段缺失则拒绝，零值用户评分与未知数据分开处理。
- 查询原文通过 queryRef=user_input 以“未核实”单独显示，不进入作品引用表。普通回答不能直接重复可识别番号/链接/已知标题；最多一次修正，再失败以 partial/answer_rejected 收尾。文字检测覆盖常见字母数字、FC2、数字型与全角/Markdown/零宽字符形式，不宣称覆盖任意语言/任何特殊番号。
- `present_movies` 要求本次请求读取本地资料，页面 ID 本身不足以生成卡片；自由 reason 被移除。写预览拒绝新引入的无依据番号，但用户明确提供的原文可以保留为草稿，不能当已验证资料。
- `run/loop.go` 不再即时转发原始 thinking/content；工具结果 UI 摘要采用服务端固定语句，避免错误体/模型参数回显。答案通过后发送正文，独立结构化提交成功即结束，不追加模型复述。保留既有默认不限工具步数、正上限及 needs_confirmation 行为。
- SSE 新增 answer_progress、15 秒注释心跳及串行响应写入；每次模型调用最多 2 分钟，累积正文与工具参数最多 256 KiB。取消、上游失败或断流不会发布草稿。Chat FirstTextMs 改为首个真正发布正文的延迟。
- message_done.answerEvidence v1 保存实际展示事实和来源元数据，复用 events_json；前端显示来源记录数量，历史恢复依据但不恢复引用权限。Web adapter 抑制旧原始 thinking_delta；Mock 改用服务端式进度回调，仍明确是假数据。

**验证**：

- Go：`go test ./...` 与 `go vet ./...` 通过；新增确定性测试覆盖假番号、分块输出、来源错配/注入、minimal、同会话并发隔离、旧引用、低步骤预算、混合终结调用、查询原文、写预览、取消与历史持久化。
- 前端：`pnpm typecheck`、`pnpm lint`、`pnpm build` 通过（没有放宽 bundle budget）。全量 Vitest 220 文件 / 1024 用例通过；之后新增真实面板进度/证据事件用例，AgentWindow 定向 27 用例通过。
- 真实模型：`TestAILiveAnswerReferences` 通过，2 次调用 / 8249 tokens。只加载当前模型与代理配置，使用临时合成 SQLite，不读取真实资料库，不注册外部 metadata/source-page 工具。验证查询→结构化提交→实际字段→历史快照。
- `pnpm test:e2e --workers=1`：4 通过 / 1 失败。失败为 `tests/e2e/personal-insights.spec.ts:44` 等待 `[data-personal-insights-page]` 超时；单独复跑仍失败。没有修改该测试、放宽断言或声称完整浏览器门禁通过。
- `go test -race` 默认因 CGO 禁用无法运行；显式启用后本机 cgo 编译器退出 2，仍无法完成。因此不声称 race 检查通过；普通并发隔离用例已通过。

**实际边界**：结构化作品字段可追溯到单一记录；源站/本地记录本身仍可能错误。解释性自然语言不是完整事实证明；未实现通用语义判断、自动校验全部推荐条件、任意网页文字升级作品事实，以及独立翻译/润色 Action 的同等发布屏障。没有新增数据库迁移/HTTP 端点/治理设置。生产安装包未重新打包，既有运行后端未重启；常规启动新代码后生效。

## 17. 上下文管理机制参考与 Curated 演进建议（2026-09-09，尚未实施）

### 17.1 结论与当前差距

本节响应用户关于成熟、先进上下文管理机制的咨询，依据当日实际读取的官方资料整理。框架机制、厂商接口状态与 Curated 的设计判断分别说明；没有进行框架横向性能实测，也不将文档示例当成效果保证。

建议参考 LangGraph 的显式会话状态与 checkpoint，以及 Anthropic 的压缩/结构化笔记/按需检索组合。Curated 继续使用 Go + SQLite + OpenAI-compatible Chat Completions，在应用层实现统一上下文编排，无需为此整体替换 Agent 框架。

**用户明确约束（2026-09-09）**：不维护针对特定提供商的优化或适配分支；原生 compaction、厂商专有 token 计数接口均不纳入实施路线。下文厂商原生能力只保留为机制研究资料，不是未来候选实现。压缩使用用户当前配置模型的普通 Chat Completions 请求，不新增摘要模型配置或供应商服务。用户认可按优先级讨论，本轮要求先通过流程图理解做法，尚未开始业务实现。

当前历史候选最近 80 条、模型窗口最多 24 条、历史正文约 24 KiB、messages/tools JSON 字节阈值 65536。旧消息被移除时没有摘要替代；跨轮主要回放 user/assistant 正文；本轮实体引用会重置。窗口大小与真实模型 token 容量尚未关联。当前 UI 的历史可回看能力不能等同于模型能够检索历史证据，也不等同于执行中 checkpoint。

### 17.2 可借鉴的正式资料

| 参考 | 已有机制与价值 | 对 Curated 的适用边界 |
| --- | --- | --- |
| LangChain / LangGraph | 显式会话 state、数据库 checkpointer、模型调用前裁剪、SummarizationMiddleware、跨会话 namespace/store | 成熟的工程组织方式，可借鉴状态模型；checkpoint 保存不自动缩短 prompt，摘要和检索需另行设计。无需引入 Python/JS 服务来复制现有 Go 工具层。 |
| Anthropic context engineering | compaction、结构化笔记、轻量引用与 just-in-time 检索、复杂任务的子 Agent 隔离 | 重点借鉴工具结果瘦身和任务状态保留；子 Agent 适合可独立研究的任务，需要额外成本及结果合并，不作为当前基础依赖。 |
| OpenAI Responses compaction | 服务端阈值触发压缩，以及独立 `/responses/compact`；压缩结果包含 opaque/encrypted item，供后续 Responses 使用 | 仅作机制参考，明确不接入；不是标准 Chat Completions 功能，压缩项也不是可跨厂商迁移的普通摘要。 |
| Anthropic 原生 compaction / context editing | 按 token 阈值生成压缩摘要，或清除旧工具结果；可结合外部 memory 按需找回 | 仅作机制参考，明确不接入；本次访问的 compaction 文档标注 Beta。 |
| Letta | 常驻 memory blocks、持久化消息、移出窗口后的历史检索，Agent 可通过工具更新记忆 | 长期有状态 Agent 的参考实现；用户偏好记忆需单独治理，不应把模型猜测直接写成用户事实。 |

### 17.3 建议的上下文组成

每次调用由应用统一组装：稳定系统规则 + 有界会话状态摘要 + 最近完整对话 + 当前页面/显式选择 + 本次所需的历史证据 + 当前执行中的工具链。用户本次输入保持完整，各部分共同受模型输入预算约束。

1. **有来源的结构化会话状态**：记录目标、用户明确约束、已完成项、待办项、未解决歧义、明确纠正，以及支撑这些内容的 message/event 引用和覆盖到的 seq。摘要是派生数据，原始记录继续保留；新摘要通过 schema/引用校验后原子替换，避免重复覆盖或遗漏。不要只写一段泛泛的“我们聊过什么”。
2. **近期完整对话与自动压缩**：保留最近若干完整轮次；接近可用预算时先去除重复或无关投影，再对更早内容做增量压缩。不能拆散待完成的 tool call/result 配对，不能把尚未执行的调用总结成已执行。压缩在工具结果完整落定的边界进行；原始依据可重新读取，避免摘要反复压缩后无法纠错。
3. **跨轮证据索引和按需读取**：对允许的只读结果保留有界事实快照、来源工具、时间、筛选、cursor/truncated 状态和稳定 evidence ID。大结果保存于服务端，模型接收简要信息及读取入口。现有 events_json 不等于完整原始工具档案，需要先核验具体字段；不能承诺可恢复未曾保存的数据。库内事实可按 movieId 重新查当前值，外部数据明确历史快照及必要刷新。
4. **明确事实、推断和权限**：摘要中的 assistant 推断不是已核实事实；歧义候选不是用户选择；历史查询结果不是本轮可信操作引用。历史 evidence ID 仅作读取线索，应用校验来源并按需重新读取后才建立本轮引用。confirm token、写权限不进入可重放记忆，成功写入状态依据持久化回执。与 §16 的发布事实引用方案保持一致。
5. **统一容量预算**：所有兼容服务走相同的请求体大小估算与软/硬预算逻辑，不维护 provider/model 容量表，不调用厂商计数接口。以可配置的应用输入预算为边界，为压缩及后续工具结果保留余量；统一估算不声称等于模型真实 tokenizer 计数。用户已知容量时可调整统一预算，未知时采用保守默认值。标准 usage 存在时只用于观察校准，不能直接代替下一次请求的预检。压缩本身也需要可用窗口、超时、取消和失败退路，不能等越界后才发起。
6. **可恢复的执行状态**：在完整模型/工具步骤边界保存目标进度及必要结果。恢复时区分已完成、未执行和执行结果未知；复用现有写入回执与幂等，不盲目重放写动作。保存聊天结束事件不能替代这个能力。

例如用户说“继续看刚才筛选结果中的第三部”：保存有序结果引用后，应用能找到第三项，再读取它的当前资料，而不是要求模型从旧回答猜 ID。长任务中压缩后仍应知道“筛选已完成、详情待查、书签还未保存”；这些状态分别来自工具结果与写入回执。

### 17.4 实施优先级与评估

建议按独立增量推进，仍遵守前文 R1 验收与事实发布门槛；本节不是启动长期记忆或新增框架的授权。

- **第一批：跨轮证据与任务状态**。补有序结果引用、只读证据回查和结构化会话状态，让续聊对象及未完成项可恢复。优先解决已观察到的“旧 ID 缺少证据、重复检索、忘记剩余步骤”。
- **第二批：预算与压缩闭环**。在完整步骤边界接入统一容量预算、摘要压缩、状态原子保存及失败回退。默认自动完成内部压缩后继续原任务；仍超限才清楚报告保留结果和恢复方式，不能把压缩当作任务完成。
- **第三批：更完整的步骤恢复与历史检索**。先按会话、实体、时间及 SQLite 文本检索选取证据；只有跨语言语义召回的评测说明有必要，再增加 embedding/向量检索。
- **后续可选：用户可控长期记忆**。提供查看、纠正、删除和禁用机制，记录来源及适用范围；普通页面筛选仍保持单次上下文。Letta/LangGraph 的 memory store 可作设计参考。

这些优先级是针对 Curated 的工程判断，不是来源给出的统一排名。应用层状态、摘要和来源记录是跨模型的可迁移基础；按照用户明确约束，不再保留 provider 专有优化路线。

验收重点：多次压缩后用户约束/纠正/未完成项仍正确；历史第三项可精确定位；过期或删除实体能够重新确认；工具失败与截断不被总结成完整成功；预览不被总结成已写入；并发请求不会互相覆盖摘要；取消/重启不重复写；摘要超时或无效时不丢旧状态；minimal 模式不会因摘要/证据回查泄露已隐藏字段。

比较旧窗口与新方案的任务完成率、事实及约束保留率、重复工具次数、压缩调用成本、总 token 用量和响应延迟。真实用量缺失继续标为未知；不以压缩率单独衡量成功，也不预先承诺收益百分比。

### 17.5 官方来源（2026-09-09 访问）

- [Anthropic：Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)
- [LangChain：Short-term memory / SummarizationMiddleware](https://docs.langchain.com/oss/python/langchain/short-term-memory)
- [LangGraph：Memory overview](https://docs.langchain.com/oss/python/langgraph/memory)
- [OpenAI：Compaction](https://developers.openai.com/api/docs/guides/compaction)
- [OpenAI：Conversation state](https://developers.openai.com/api/docs/guides/conversation-state)
- [Anthropic：Compaction（页面标注 Beta）](https://platform.claude.com/docs/en/build-with-claude/compaction)
- [Anthropic：Context editing](https://platform.claude.com/docs/en/build-with-claude/context-editing)
- [Letta：Memory / stateful agents](https://docs.letta.com/guides/agents/memory/)

本次仅追加参考与建议，不修改业务代码、不调用付费模型、不执行部署；未运行构建或测试。前文和工作区已有改动保留。

### 17.6 面向讨论的整体流程（2026-09-09，方案示意）

核心职责：Curated 决定保存什么、选取什么、何时压缩；配置中的模型接收正常聊天请求，负责推理与必要的摘要生成。查询事实和写入结果以工具及回执为依据。

本地信息分三类管理，并不表示必须新建三个数据库或服务：

| 类别 | 保存内容 | 进入模型的方式 |
| --- | --- | --- |
| 会话记录 | 用户/助手正文，必要工具事件及来源快照 | 最近完整对话直接携带；更早内容按需读取 |
| 任务笔记 | 当前目标、明确要求、已完成/待办、用户纠正、引用及摘要覆盖范围 | 以有界摘要随任务携带；非所有会话永久注入 |
| 证据索引 | 已展示列表的顺序和身份、查询结果引用、时间、来源、截断范围 | 当前相关结果携带简要信息；详情用工具读取 |

```mermaid
flowchart TD
    userInput["用户发送新消息"] --> assemble["Curated 组装本次上下文"]
    localStore[("本地：会话记录、任务笔记、证据索引")] -.-> assemble
    assemble --> budget{"是否需要腾出空间？"}
    budget -->|"否"| modelCall["通过统一聊天接口调用当前模型"]
    budget -->|"是"| compact["精简旧结果，必要时生成并校验摘要"]
    compact --> ready{"整理后可继续？"}
    ready -->|"是，保存有效新状态"| modelCall
    ready -->|"否"| pause["保留原状态，说明原因与恢复方式"]
    modelCall --> toolNeeded{"需要读取资料或执行工具？"}
    toolNeeded -->|"是"| toolRun["执行工具，保存必要证据并更新进度"]
    toolRun --> assemble
    toolRun -.-> localStore
    toolNeeded -->|"否"| reply["发布回答，保存本轮记录和状态"]
    reply -.-> localStore
```

这张图聚焦上下文循环；需要用户选择/写入确认、取消和工具失败沿用现有控制流程，不表示工具可以自动越过确认。每次工具执行前仍检查实际权限；历史摘要不能成为授权来源。动态页面信息每轮重新获取，不因被提及就永久写入偏好。

**第一批的可见变化**：用确定性代码记录已展示结果顺序、工具成功/失败和真实写入状态，先让“刚才第三部”“哪些已完成”有据可查。用户目标和约束先保留有界原文/消息引用，任务笔记中的语义归纳是待核验的派生信息，不能用它覆盖用户原话。不会为了保存每条笔记都额外调用模型。

**第二批的整理过程**：只在接近软预算、且一组工具调用结果已完整落定时整理。先去掉重复和不相关的模型投影；仍不足时，用同一个当前模型、同一个普通聊天接口，对旧笔记和新增旧记录生成结构化摘要。工具状态由程序合并，近期对话、本次输入及关键用户约束保留；长历史分段处理，避免摘要请求自身超限。输出不依赖厂商 strict JSON 特性，由本地解析及引用校验把关；格式正确不代表语义绝对正确，仍需针对要求/纠正保留做评测。失败或摘要并未减少占用时不覆盖旧状态、不无限重试；原状态能安全容纳请求则沿用，否则清楚暂停。

**具体例子**：用户要求“找 5 部未观看且不超过 90 分钟的作品，选好后存书签”。第一次查询后保存的是已实际展示的有序列表及其证据，区分搜索命中、候选和最终展示。用户说“第三部换掉”时，定位该列表第三项；若多个列表导致指代不清，进行澄清。若期间压缩，笔记保留目标、用户限制、已展示列表引用、排除项和“书签尚未保存”。最后只有持久化成功回执能将保存步骤更新为完成。

**用户体验与成本**：普通短对话沿用正常调用；真正需要摘要时才有额外模型请求和等待，可显示简短“正在整理上下文”状态并允许取消。压缩成功后继续原任务，不要求用户重复目标或手动新建会话。整理只改变发送给模型的投影，不通过压缩删除原聊天；主动删除会话时应同步清理它的笔记及证据。跨进程完整步骤恢复属于第三批，不能因保存了摘要就提前宣称已具备。

本节仅细化流程并落实提供商无关约束，未开始业务开发。

### 17.7 上下文管理实施计划（待用户确认，2026-09-09）

#### A. 本次确认范围与目标

用户要求先给实施计划，确认后再实施。本节为 §17 的执行依据；本轮只更新本文件，不修改代码、配置或数据库，不调用真实模型、不提交工作区其他改动、不启动服务或发布。

确认后按“准备与基线 → 第一阶段跨轮证据/任务状态 → 第二阶段统一预算/自动压缩 → 第三阶段中断恢复/历史检索 → 集成验收”顺序推进。每阶段形成可独立验证的交付，不把内部表建完或单测通过直接称为用户功能完成。

固定约束：

- 继续 Go + SQLite + Vue，复用影片库的同一个 SQLite 文件，AI 数据独立表及生命周期；不增加数据库服务或 Agent 框架。
- 所有模型走现有 OpenAI-compatible Chat Completions；摘要使用当前配置模型，不引入厂商专有 API、模型容量目录或独立摘要模型配置。
- 保留本次输入、当前任务的明确约束与最新纠正；原聊天不因压缩被删除。过多原文无法全部装入窗口时保留来源引用并按需回查，不承诺无限常驻。
- 新旧证据都不能绕过现有事实发布、权限检查、写入预览/确认/回执。页面筛选和用户明确提出的任务约束分开，页面上下文仍按本轮实际页面获取。
- 本次交付包括三个阶段；跨会话偏好记忆、向量库、子 Agent 并行、自动后台继续任务不在此次范围内。产品打包、生产数据库迁移和发布另按发布流程执行。

#### B. 准备工作：核定接入基线

1. 对照当前 HEAD 与工作区差异，记录 Agent 的工具、终态、事实引用和 SSE 实际接口。2026-09-09 检查时工作区已出现 `AnswerRefStore`、`submit_answer`、回答发布校验及请求 scope 改动，不能再沿用旧“仅按 session 分桶”的假设，也不能覆盖其他任务改动。
2. 对已衔接功能先跑相关基线测试；若有已有失败，记录归因，避免将旧问题算作本次回归。前文 §16 的完整发布方案不自动扩张成本计划必做范围，但实际已启用的发布规则必须兼容。
3. 冻结本次内部 DTO：会话状态版本、证据身份、显示列表顺序、摘要覆盖水位、运行步骤状态。所有读写以 session/run 身份限定，禁止将 `refId` 直接持久化成可跨请求授权的凭证。
4. 明确同一会话一次只允许一个生成请求：首版建议重复请求返回明确 busy/conflict 状态，不持有数据库连接排队；不同会话独立运行。会话删除先阻止该会话继续写入并取消活动请求，删除与晚到结果的竞争必须可测。

交付：确定的合约、接入位置、基线结果；不另外开一条提供商适配路线。

#### C. 第一阶段：续聊证据与任务状态

**用户可见结果**：“刚才第三部换掉”“已经查到哪些”“继续之前没完成的部分”能够找到真实对象和执行依据。

建议在同一个数据库增加以下逻辑表，具体迁移编号在实现当天分配，避免与并行改动冲突：

| 建议表 | 关键内容 | 生命周期 |
| --- | --- | --- |
| `ai_session_context` | session_id、schema_version、revision、目标/约束消息引用、已完成/待办状态、摘要、已覆盖消息 seq 与 run/step 水位、updated_at | 每会话一份有效快照；删除会话级联删除 |
| `ai_context_evidence` | evidence_id、session/run、工具及来源身份、查询时间、筛选/截断范围、有界事实快照、关联事件 | 会话所属只读依据；不可存确认 token 或写权限 |
| `ai_context_presentations` | presentation_id、session/run、实际发布顺序、有序 evidence ID/实体身份、展示状态 | 区分搜索命中、候选与实际发布列表；用于“第三项”回指 |

实施内容：

1. 增加 storage repository 和索引：以 session/run、消息顺序、证据身份定位，分页取必要列；不在列表查询里加载全部 JSON。revision 比较更新，摘要生成前读快照、生成后短事务提交，旧版本不能覆盖新版本。
2. 只对允许的工具结果建立有界事实记录；复用/衔接现有 AnswerRef 字段和来源逻辑，区分持久化证据 ID 与当次可用的回答引用。模型推断、推荐理由、未确认候选不能被提取为已核实事实。
3. 在最终发布路径保存列表身份与顺序。服务端能证明的是已持久化/已发布的列表，不保证客户端断线时用户一定看到；不完整流明确标记，不能当作完整已展示列表自动选第三项。
4. 增加只读内部工具 `get_context_evidence`，按本会话 evidence/presentation ID 分页读取。注入最近相关列表的简短目录；更早全文检索放到第三阶段。重新操作库内影片时读取当前资料并生成本轮引用；对象已删除或来源已失效时明确返回。
5. 程序记录已执行步骤、工具成功/失败、预览状态和成功回执；目标/要求先保留原文引用，不为每次工具结果额外调用模型。任务语义归纳不自动证明任务完成。
6. 将组装逻辑集中到拟新增的 `backend/internal/agent/run/context_manager.go`，通过窄接口调用 app/storage；run 不直接依赖 SQLite。按现有角色边界注入摘要与外部数据，不把派生摘要或工具正文提升为系统指令。
7. 旧会话继续可读：只从已有可靠事件恢复结果顺序与事实，缺失的原始依据不伪造；必要时重新查询。删除会话清理新增表，不删除影片或撤销已写入书签。

验收：跨轮/重开会话能定位已发布列表第三项；多列表歧义不擅自选取；旧来源、删除实体、截断/失败结果可区分；跨会话证据 ID 被拒绝；minimal 投影不泄露隐藏字段；删除会话不影响影片和已保存业务结果；普通短会话没有额外摘要模型调用。

#### D. 第二阶段：统一预算与自动压缩

**用户可见结果**：长对话或长工具任务接近容量时显示“正在整理上下文”，成功后继续原请求；用户仍可停止。

实施内容：

1. 把当前名称容易误导的 token estimate 明确为请求 JSON UTF-8 字节估算。共用一个应用输入预算，不按提供商分支。建议初始硬预算沿用 64 KiB，软整理阈值为其 75%，整理目标约 50%；这些是待合成轨迹验证的应用默认值，不是模型真实 token 容量，也不是永远固定的产品值。
2. 提供一个高级“上下文输入预算”配置和功能回退开关，软阈值/目标由程序推导，避免多个相互矛盾的设置。扩大输入预算仍受 HTTP 请求和存储边界约束；不以调整预算伪装精确支持某模型容量。
3. 组装统一计算系统规则、工具 schema、状态、近期完整对话、本次输入、必要证据及当前工具链的占用。近期窗口改由预算和完整轮次共同决定；旧的 24 条裁剪不能继续先丢记录、再尝试摘要。摘要前能按已覆盖水位分页获取尚未处理的旧消息，不受最近 80 条候选限制。
4. 先精简重复、无关的模型投影，再摘要。只有一组工具调用/结果完整落定后才压缩；当前请求中尚未进入消息表的完整步骤也必须纳入来源水位，避免遗漏或重复摘要。本次输入不截断。
5. 用相同当前模型和普通聊天接口生成结构化摘要，禁止摘要调用执行业务工具；本地检查结构、长度、引用归属和版本，并由程序合并可靠执行状态。工具来源仍是数据，摘要不得提取系统指令或恢复确认权限。格式通过不代表语义必然正确，需配套真实模型评测。
6. 摘要生成不占数据库连接；长积累按有界分段处理，摘要请求自身也预检。一次候选失败不无限重试；输出无效、无压缩收益、取消、版本过期或超时均不替换旧状态。还能安全调用时沿用旧状态，否则返回明确可恢复的 partial/needs_input。
7. 按现有服务层扩展可选 `context_status` SSE 事件和历史状态 DTO，Web/Mock、组件、中英日文案同时更新。新增状态仅展示整理/完成/失败，不展示内部摘要全文或原始隐私数据。心跳与上游期限分开，整理不能被客户端 90 秒空闲错误打断，持续无响应仍需结束。
8. 用现有运行观测记录摘要调用次数、耗时、估算前后体积和提供方返回的 usage，标明摘要用途；缺失计数为未知。新增摘要调用服从当前 AI 启用、取消和脱敏策略，密钥/模型配置在单次运行内采用一致快照。

验收：超过 80 条历史及至少三次压缩后仍遵守早期要求与最新纠正；处理中工具 JSON 不被切断；压缩后接续原目标；超大本次输入明确拒绝/提示而不是暗截断；无效摘要、失败、取消、无收益和删除竞争不丢旧状态；原聊天仍可回看；同一个 fake OpenAI-compatible 服务验证全部协议，不依赖扩展字段。

#### E. 第三阶段：中断恢复与历史检索

**用户可见结果**：重新打开后能看见任务在哪里中断，通过“继续”恢复未完成事项；可以找回更早的相关记录。

1. 新增运行/步骤持久化（建议 `ai_context_runs`、`ai_context_steps`）：run/session、步骤唯一标识、started/completed/interrupted/unknown、来源水位及必要输出引用。每个步骤准备/完成边界短事务写入，不对每个 SSE delta 写库，不保存可重放的确认 token 或任意敏感工具参数。
2. 后端重启后把未完成运行标成 interrupted/unknown；UI 提供显式继续入口，不在后台自动重新执行。取消的任务也不会自动重启。恢复使用新 run、读取最新治理设置，并只接续已保存的完整步骤。
3. 只读操作可在必要时重新查询。写入结果先以现有持久化回执和当前资料核对；结果未知时不盲目重放，也不声称 exactly-once。旧预览不恢复可用确认票据，需要时重新预览确认。
4. 新增本会话只读 `search_context_history`，按消息、时间、实体和关键字返回分页片段及来源引用，随后通过证据工具读取。先采用 SQLite 可支持的文本查询和索引；不增加 embedding 服务，不默认跨会话检索。明确历史记录是当时的事实快照。
5. 历史流与恢复结果采用稳定 run/step/event ID 去重，避免恢复后显示两份同样结果；上下文状态、水位与保存的完成步骤一致。

验收：在模型前、工具后保存前、完整步骤保存后、写入已成功但响应丢失等位置注入中断；不虚构已完成、不自动重复写入；主动取消不会后台继续；旧预览不可应用；重启后可继续读取已保存的任务和证据。

#### F. 共用数据库的容量、并发与兼容

- 保持当前数据库连接与 journal 策略；首版不为本功能擅自提高连接数或切换 WAL。发现瓶颈先通过测量定位。
- 所有模型、网络调用都在事务外；按完整步骤批量保存。会话状态使用版本比较更新，按 session/run 范围读写；同会话生成串行化不能代替删除、确认回调和异步保存的竞争检查。
- 每类证据设单条、单页和单会话容量上限。首版建议单条 16 KiB、检索单页 20 项、每会话派生上下文总量 4 MiB（不含原聊天与既有回执）；这些为工程起点，基准测试后定稿。裁剪必须按字段/条目边界处理，并保留 truncated 标记。触顶先清理可重建旧证据正文，保留有界身份目录；仍不能保存则明确容量状态，不静默覆盖当前引用或形成无限目录。
- 原聊天按现有会话生命周期保存，自动压缩不删除它；新增笔记/证据/运行步骤随会话删除。资料库当前事实依然以业务表为准。清理不执行大范围同步 VACUUM，删除行也不承诺数据库文件立即缩小。
- 迁移仅新增表/索引或可选字段；旧消息和影片表不重写。升级用临时数据库副本验证；功能开关关闭时回到原聊天组装路径，新表可保留。二进制降级兼容单独测试，不把开关回退等同于撤销迁移。
- 复用现有备份流程验证新表可备份/恢复；本计划不授权对正式库做恢复或删除操作。

#### G. 测试与交付标准

先在临时 SQLite 和 scripted streamer 下完成确定性测试，再对当前配置模型做小规模合成场景兼容评测；不把私人影片库作为测试数据。真实模型测试需要按项目现有显式评测开关执行，报告模型调用及实际可得 usage，不自动扩大为多厂商矩阵。

| 检查 | 通过条件 |
| --- | --- |
| 续聊准确性 | 已展示对象、次序、早期条件、最新纠正在指定 fixture 中全部正确；歧义不擅自消解 |
| 摘要可靠性 | 至少三次压缩后目标/待办/纠正仍保留；保存和恢复水位没有遗漏/重复；失败不覆盖旧状态 |
| 权限与来源 | 跨会话、过期引用、摘要伪造事实和旧确认均不能突破现有工具与发布校验 |
| 共库响应 | 同一合成影片库下比较 AI 关闭与多个独立会话活动时的列表/详情延迟、错误和写锁等待；模型故意等待时影片 API 可完成，不随模型等待时间阻塞；数据准备和缓存条件一致，记录 p50/p95，不凭主观宣称无影响 |
| 容量与清理 | 达到单条/会话上限时有明确截断/降级；删除会话不删影片，晚到写入不复活会话 |
| 中断恢复 | 多个注入中断点可解释状态；成功写入不重复执行；取消后保持停止 |
| 前端兼容 | Web/Mock 与旧会话均可用；整理状态可取消，恢复入口去重，文案与实际状态一致 |

命令遵守 `docs/ops/2026-04-08-agent-build-and-test.md`：

- 在 `backend/`：先跑涉及的 `go test ./internal/agent/... ./internal/storage/... ./internal/app/... ./internal/server/... ./internal/llm/...`；集成完成后 `go test ./...`、`go vet ./...`。
- 仓库根目录：定向 Vitest 覆盖 Agent Window、历史恢复、Web/Mock adapter 和新增设置；集成后 `pnpm typecheck`、`pnpm lint`、`pnpm test`、受影响的 `pnpm test:e2e` 及 `pnpm build`。不放宽 bundle budget，不运行无关 display 长套件；Electron 专项仅在壳层行为受影响时增加。
- 记录新旧实现同一合成任务的完成情况、重复查询次数、摘要额外调用/耗时/usage、SQLite 增长和页面响应。第一轮性能基线完成后才设可重复的数值回归阈值；确认计划不等于已经通过性能验收。

#### H. 提交顺序、文档与确认点

按独立行为组织提交：存储及数据生命周期 → 跨轮证据读取与发布衔接 → 状态组装与旧会话兼容 → 预算/摘要闭环及必要 UI → 步骤恢复及必要 UI → 历史检索 → 集成验证与文档。每单元携带对应测试，跨层紧耦合行为同批提交，不提交别的任务改动，不 push。

实现后同步 `.cursor/rules/project-facts.mdc`、必要 quick reference、README 短入口、`docs/guide.md`、`CLAUDE.md`、`docs/reference/architecture-and-implementation.html`；公开事件/端点变更更新 `API.md`，新增 library-config 治理字段同时更新 `docs/reference/2026-03-21-library-organize.md`。只有实际落地并验证的阶段才修改事实状态。

本轮请用户确认的是：三个阶段的范围、统一接口/共库的架构、先续聊后压缩再恢复的顺序，以及上述完成标准。确认前不执行任何业务实施；确认后按阶段持续完成并报告验收，不再对常规实现选择反复请求批准。
