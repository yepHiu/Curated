...
| 2.3 CuratedFramesLibrary 拆分 | 进行中 | 已抽出批量导出选择 reducer `curated-frames/selection`，覆盖清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推；已抽出单帧卡片 `CuratedFrameCard`、三视图共用网格 `CuratedFrameGrid`、演员 tab `CuratedFrameActorsTab` 与影片 tab `CuratedFrameMoviesTab`，复用 timeline / actors / movies 三处卡片布局、选择态、近重复态和事件转发，并收口 actors/movies 分组头渲染/勾选事件；组件继续负责 toast/i18n 与视图分组数据 | `src/components/jav-library/CuratedFramesLibrary.vue`, `src/components/jav-library/CuratedFrameCard.vue`, `src/components/jav-library/CuratedFrameCard.test.ts`, `src/components/jav-library/CuratedFrameGrid.vue`, `src/components/jav-library/CuratedFrameGrid.test.ts`, `src/components/jav-library/CuratedFrameActorsTab.vue`, `src/components/jav-library/CuratedFrameActorsTab.test.ts`, `src/components/jav-library/CuratedFrameMoviesTab.vue`, `src/components/jav-library/CuratedFrameMoviesTab.test.ts`, `src/lib/curated-frames/selection.ts`, `src/lib/curated-frames/selection.test.ts` |
...
**状态（2026-05-01）:** 进行中。已抽出 `src/lib/curated-frames/selection.ts`，将批量导出选择的清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推从 `CuratedFramesLibrary.vue` 中移出，并补 `src/lib/curated-frames/selection.test.ts`；已抽出 `src/components/jav-library/CuratedFrameCard.vue`，将重复单帧卡片 UI、批量勾选、近重复 badge、打开 dialog 与右键菜单事件转发收口，并补 `src/components/jav-library/CuratedFrameCard.test.ts`；已抽出 `src/components/jav-library/CuratedFrameGrid.vue`，复用 timeline / actors / movies 三处网格渲染、选择态/近重复态注入和事件转发，并补 `src/components/jav-library/CuratedFrameGrid.test.ts`；已抽出 `src/components/jav-library/CuratedFrameActorsTab.vue`，将演员分组标题、批量分组勾选和 actor section 事件转发从父组件移出，并补 `src/components/jav-library/CuratedFrameActorsTab.test.ts`；已抽出 `src/components/jav-library/CuratedFrameMoviesTab.vue`，将影片分组标题/副标题、批量分组勾选和影片 tab 事件转发从父组件移出，并补 `src/components/jav-library/CuratedFrameMoviesTab.test.ts`。组件仍保留分页、dialog tag 编辑、导出/删除 orchestration 与 timeline tab UI。
