...
| 5.1 硬编码中文迁移到 locale 文件 | 部分完成 | `PlayerView` 的 loading / not found 可见文本已迁移到 `player.*`；目录选择提示已迁移到 `pickDir.*`；相关路径由 `PlayerView.test.ts`、`pick-directory.test.ts` 与 locale parity 测试覆盖 | `src/views/PlayerView.vue`, `src/views/PlayerView.test.ts`, `src/lib/pick-directory.ts`, `src/lib/pick-directory.test.ts`, `src/locales/en.json`, `src/locales/ja.json`, `src/locales/zh-CN.json` |
...
3. **5.1 硬编码文本迁移**：继续处理 preview / scan dock / rating 等可见文本。
...
**状态（2026-05-01）:** 部分完成。`PlayerView.vue` 的 `player.loadingTarget`、`player.notFoundTitle`、`player.notFoundDesc` 已迁移并补齐三语文案；`pick-directory.ts` 的目录选择提示已迁移到 `pickDir.unsupported` / `pickDir.selected` 并补齐三语文案；其余 preview、scan、rating 等文本仍需继续。
