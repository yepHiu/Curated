...
| 2.3 CuratedFramesLibrary 拆分 | 进行中 | 已抽出批量导出选择 reducer `curated-frames/selection`，覆盖清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推；已抽出单帧卡片 `CuratedFrameCard`，复用 timeline / actors / movies 三处卡片 UI 和事件转发；组件继续负责 toast/i18n 与视图分组 | `src/components/jav-library/CuratedFramesLibrary.vue`, `src/components/jav-library/CuratedFrameCard.vue`, `src/components/jav-library/CuratedFrameCard.test.ts`, `src/lib/curated-frames/selection.ts`, `src/lib/curated-frames/selection.test.ts` |
...
### 2.3 CuratedFramesLibrary 拆分（1720 行）

**状态（2026-05-01）:** 进行中。已抽出 `src/lib/curated-frames/selection.ts`，将批量导出选择的清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推从 `CuratedFramesLibrary.vue` 中移出，并补 `src/lib/curated-frames/selection.test.ts`；已抽出 `src/components/jav-library/CuratedFrameCard.vue`，将 timeline / actors / movies 三处重复单帧卡片 UI、批量勾选、近重复 badge、打开 dialog 与右键菜单事件转发收口，并补 `src/components/jav-library/CuratedFrameCard.test.ts`。组件仍保留分组、分页、dialog tag 编辑、导出/删除 orchestration 与 tab UI。
