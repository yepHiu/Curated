...
| 2.2 PlayerPage 拆分 | 进行中 | 已先抽出播放统计格式化 helper `player-playback-stats-format` 与时间轴 helper `player-playback-timeline`，覆盖 bitrate、resolution、fps、timecode、percent、count、reason、session kind、source format、transcode profile、unavailable 判定、时钟显示、总时长解析、HLS offset、绝对时间、进度目标归一化与 seek clamp；`PlayerPage.vue` 保留播放状态编排和 UI 引用 | `src/components/jav-library/PlayerPage.vue`, `src/lib/player-playback-stats-format.ts`, `src/lib/player-playback-stats-format.test.ts`, `src/lib/player-playback-timeline.ts`, `src/lib/player-playback-timeline.test.ts` |
...
### 2.2 PlayerPage 拆分（2681 行）

**状态（2026-05-01）:** 进行中。已抽出 `src/lib/player-playback-stats-format.ts`，将播放统计面板里的 bitrate / resolution / fps / timecode / percent / count / reason / session kind / source format / transcode profile / unavailable 判定从 `PlayerPage.vue` 中移出，并补 `src/lib/player-playback-stats-format.test.ts` 覆盖缺失值、clamp、profile 映射与格式化边界；已抽出 `src/lib/player-playback-timeline.ts`，将时钟显示、总时长解析、HLS offset、绝对播放时间、进度目标归一化与 seek 目标 clamp 从页面内纯逻辑中移出，并补 `src/lib/player-playback-timeline.test.ts`。`PlayerPage.vue` 当前仍保留 media element 生命周期、HLS session、进度上报、键盘/沉浸 chrome 与截图编排逻辑。
