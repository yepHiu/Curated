...
状态：verified（仅本节体积监管；原优化路线其余阶段不因此升级状态）

用户要求允许新功能合理增长，避免旧 2.25 MB 总 JS 硬预算频繁卡住构建，同时防止无限增长。本次替换原硬预算：正常放行 → 关注/增长提醒 → 绝对严重超限阻断。不是定时监控；每次本地构建和现有 CI 自动评估。

- 统一配置 `bundle-policy.json`：首屏 JS raw/gzip 关注 900/300 kB、严重 1500/500 kB；全部 JS 关注 5000/1600 kB、严重 8000/2500 kB；CSS 关注 500 kB、严重 1500 kB；前端总量关注 100 MB、严重 150 MB。宽松上限是当前管理策略，不是性能达标保证。
- 最近主分支前端质量作业成功快照观察单次增长（20% 且超过噪声门槛）；人工基线观察长期累计增长（50% 且超过两倍噪声门槛）。两者均不阻断；绝对上限固定，不随基线自动增长。允许正常迭代而不会无限放宽。
- 单块预算与必需 chunk 名称限制取消；大分块和 HLS 首屏静态依赖仅提醒。继续保持按需加载设计，不主动改业务分块。
- 报告统计最终文件，包括 public、字体、JSON 和 CSS，排除报告自身；严重失败前仍保存 JSON/Markdown。CI Summary、Annotations 与 90 天 Artifact 提供通知及历史对照。只保存主分支成功的前端质量作业缓存，PR 不更新；缓存缺失/损坏/模式不同显式降级，绝对上限不失效。
- 人工基线只在审阅新鲜 Web 生产报告后通过 `pnpm bundle:baseline` 更新，不自动执行。当前初始实测约首屏 411.3 kB raw / 145.4 kB gzip、全部 JS 2.26 MB raw / 699.6 kB gzip、CSS 319.4 kB、前端总量 33.62 MB。必需字体占主要资源体积，无需为了满足旧 JS 阈值删减功能。
- 此次范围为现有 Vue 前端构建，不包含 Electron、Go、FFmpeg 和 Windows 安装包；未来增加这些指标必须固定平台和构建方式，不能把跨平台数据混合比较。

验证：专用 5 项测试覆盖合理增长/缩小、绝对噪声门槛、突增、累计漂移、所有指标的阈值边界、Web/Mock 隔离、非法基线与策略；真实 Vite 临时项目验证提醒放行、静态/动态 JS、CSS/public 统计、严重超限时报告落盘后失败。生产 Web 构建、类型检查与 ESLint 通过。初次使用 `pnpm test -- <file>` 时意外触发全套测试，Node 25 的 localStorage 行为使无关测试失败；已停止该轮，改为 `pnpm test <file>` 后专项通过，未宣称全套回归通过。操作及阈值详见 [构建手册](../ops/2026-04-08-agent-build-and-test.md)。GitHub 托管 CI 需推送后执行验证。
...
| 2.1 SettingsPage 拆分 | 进行中 | 已抽出低耦合的总览统计区 `SettingsOverviewSection`、通用设置区 `SettingsGeneralSection`、资料库整理区 `SettingsOrganizeSection`、网络代理区 `SettingsNetworkSection`、元数据大区 `SettingsMetadataSection`、元数据自动化区 `SettingsMetadataAutomationSection`、元数据模式区 `SettingsMetadataModeSection`、单 Provider 选择区 `SettingsMetadataProviderSelectSection`、多源 Provider 链管理区 `SettingsMetadataProviderChainSection`、手动触发全库刮削区 `SettingsMetadataTriggerScrapeSection`、资料库路径大区 `SettingsLibraryPathsSection`、资料库路径批量工具条 `SettingsLibraryPathToolbar`、资料库路径列表 `SettingsLibraryPathList`、资料库路径新增弹窗 `SettingsLibraryPathAddDialog`、资料库路径移除确认弹窗 `SettingsLibraryPathRemoveDialog`、策展帧设置区 `SettingsCuratedSection`、维护工具区 `SettingsMaintenanceSection` 与关于页 `SettingsAboutSection`，父组件仅保留路由 tab、状态计算与事件转发；新增组件测试覆盖 dashboard stats 渲染、locale/theme/launch-at-login 事件、organize switch 事件、proxy 多字段 v-model 与保存/连通性测试动作、metadata section 标题/指定 provider 健康/链模式事件转发、provider 全量 ping 与自动刮削开关、metadata provider mode 三选一、metadata provider select 选项/当前站点 ping/健康状态渲染、metadata provider chain 渲染/新增/保存/ping/remove/空态错误态、trigger scrape 运行/忙碌/成功/失败态、library paths section 卡片/反馈态/事件转发、library path toolbar 批量管理/全选/清空/刷新/退出与禁用态、library path list 普通行/编辑行/批量勾选/行动作转发、library path add dialog 字段更新/选择目录/保存禁用/错误提示、library path remove dialog 详情/忙碌禁用/open 更新/confirm、curated 保存策略/导出格式/目录动作、full scan 事件、busy 禁用态、Web API 版本区与首页推荐刷新事件 | `src/components/jav-library/SettingsPage.vue`, `src/components/jav-library/settings/SettingsOverviewSection.vue`, `src/components/jav-library/settings/SettingsOverviewSection.test.ts`, `src/components/jav-library/settings/SettingsGeneralSection.vue`, `src/components/jav-library/settings/SettingsGeneralSection.test.ts`, `src/components/jav-library/settings/SettingsOrganizeSection.vue`, `src/components/jav-library/settings/SettingsOrganizeSection.test.ts`, `src/components/jav-library/settings/SettingsNetworkSection.vue`, `src/components/jav-library/settings/SettingsNetworkSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataSection.vue`, `src/components/jav-library/settings/SettingsMetadataSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataAutomationSection.vue`, `src/components/jav-library/settings/SettingsMetadataAutomationSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataModeSection.vue`, `src/components/jav-library/settings/SettingsMetadataModeSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataProviderSelectSection.vue`, `src/components/jav-library/settings/SettingsMetadataProviderSelectSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataProviderChainSection.vue`, `src/components/jav-library/settings/SettingsMetadataProviderChainSection.test.ts`, `src/components/jav-library/settings/SettingsMetadataTriggerScrapeSection.vue`, `src/components/jav-library/settings/SettingsMetadataTriggerScrapeSection.test.ts`, `src/components/jav-library/settings/SettingsLibraryPathsSection.vue`, `src/components/jav-library/settings/SettingsLibraryPathsSection.test.ts`, `src/components/jav-library/settings/SettingsLibraryPathToolbar.vue`, `src/components/jav-library/settings/SettingsLibraryPathToolbar.test.ts`, `src/components/jav-library/settings/SettingsLibraryPathList.vue`, `src/components/jav-library/settings/SettingsLibraryPathList.test.ts`, `src/components/jav-library/settings/SettingsLibraryPathAddDialog.vue`, `src/components/jav-library/settings/SettingsLibraryPathAddDialog.test.ts`, `src/components/jav-library/settings/SettingsLibraryPathRemoveDialog.vue`, `src/components/jav-library/settings/SettingsLibraryPathRemoveDialog.test.ts`, `src/components/jav-library/settings/SettingsCuratedSection.vue`, `src/components/jav-library/settings/SettingsCuratedSection.test.ts`, `src/components/jav-library/settings/SettingsMaintenanceSection.vue`, `src/components/jav-library/settings/SettingsMaintenanceSection.test.ts`, `src/components/jav-library/settings/SettingsAboutSection.vue`, `src/components/jav-library/settings/SettingsAboutSection.test.ts` |
| 2.2 PlayerPage 拆分 | 进行中 | 已先抽出播放统计格式化 helper `player-playback-stats-format`、时间轴 helper `player-playback-timeline` 与播放统计 reducer `player-playback-stats`，覆盖 bitrate、resolution、fps、timecode、percent、count、reason、session kind、source format、transcode profile、unavailable 判定、时钟显示、总时长解析、HLS offset、绝对时间、进度目标归一化、seek clamp、HLS level/bandwidth/fragment stats 归一化与平滑；`PlayerPage.vue` 保留播放状态编排和 UI 引用 | `src/components/jav-library/PlayerPage.vue`, `src/lib/player-playback-stats.ts`, `src/lib/player-playback-stats.test.ts`, `src/lib/player-playback-stats-format.ts`, `src/lib/player-playback-stats-format.test.ts`, `src/lib/player-playback-timeline.ts`, `src/lib/player-playback-timeline.test.ts` |
| 2.3 CuratedFramesLibrary 拆分 | 进行中 | 已抽出批量导出选择 reducer `curated-frames/selection`，覆盖清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推；已抽出单帧卡片 `CuratedFrameCard`、三视图共用网格 `CuratedFrameGrid`、时间线 tab `CuratedFrameTimelineTab`、演员 tab `CuratedFrameActorsTab`、影片 tab `CuratedFrameMoviesTab`、tag filter bar `CuratedFrameTagFilterBar`、顶部工具栏 `CuratedFrameLibraryToolbar`、删除确认框 `CuratedFrameDeleteConfirmDialog` 与空状态 `CuratedFrameEmptyState`，复用 timeline / actors / movies 三处卡片布局、选择态、近重复态和事件转发，并收口 actors/movies 分组头、tag facet、tab/batch toolbar、删除确认与空状态渲染/事件；组件继续负责 toast/i18n 与视图分组数据 | `src/components/jav-library/CuratedFramesLibrary.vue`, `src/components/jav-library/CuratedFrameCard.vue`, `src/components/jav-library/CuratedFrameCard.test.ts`, `src/components/jav-library/CuratedFrameGrid.vue`, `src/components/jav-library/CuratedFrameGrid.test.ts`, `src/components/jav-library/CuratedFrameTimelineTab.vue`, `src/components/jav-library/CuratedFrameTimelineTab.test.ts`, `src/components/jav-library/CuratedFrameActorsTab.vue`, `src/components/jav-library/CuratedFrameActorsTab.test.ts`, `src/components/jav-library/CuratedFrameMoviesTab.vue`, `src/components/jav-library/CuratedFrameMoviesTab.test.ts`, `src/components/jav-library/CuratedFrameTagFilterBar.vue`, `src/components/jav-library/CuratedFrameTagFilterBar.test.ts`, `src/components/jav-library/CuratedFrameLibraryToolbar.vue`, `src/components/jav-library/CuratedFrameLibraryToolbar.test.ts`, `src/components/jav-library/CuratedFrameDeleteConfirmDialog.vue`, `src/components/jav-library/CuratedFrameDeleteConfirmDialog.test.ts`, `src/components/jav-library/CuratedFrameEmptyState.vue`, `src/components/jav-library/CuratedFrameEmptyState.test.ts`, `src/lib/curated-frames/selection.ts`, `src/lib/curated-frames/selection.test.ts` |
...
| 5.2 性能微优化 | 已完成 | `HomeView` 对播放进度 revision 引发的 portal model 重建加 5s 防抖，避免播放期间高频重算；`ScanProgressDock` 静态统计标签、`DetailPage` 空预览占位块、`LibraryBatchActionBar` 批量确认弹窗静态标题加 `v-once`；详情页预览图仅优先加载首屏 4 张且仅第 1 张 `fetchpriority=high`，后续预览图 lazy/low；预览大图查看器按选中图/邻近图调度 eager，其他大图与缩略图 lazy/low，并在主图 load 前保留占位和淡入；新增 render hints 与图片加载策略回归 | `src/views/HomeView.vue`, `src/views/HomeView.test.ts`, `src/components/jav-library/ScanProgressDock.vue`, `src/components/jav-library/DetailPage.vue`, `src/components/jav-library/DetailPage.test.ts`, `src/components/jav-library/PreviewImageViewerInner.vue`, `src/components/jav-library/PreviewImageViewerInner.test.ts`, `src/components/jav-library/LibraryBatchActionBar.vue`, `src/components/jav-library/render-hints.test.ts` |
...
- `pnpm test -- src/lib/curated-frames/selection.test.ts`：1 file / 5 tests passed
- `pnpm test -- src/lib/curated-frames/selection.test.ts src/lib/curated-frames/capture.test.ts src/lib/curated-frames/db.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：5 files / 20 tests passed
- `pnpm test -- src/lib/curated-frames/selection.test.ts src/lib/curated-frames/capture.test.ts src/lib/curated-frames/db.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：5 files / 21 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameCard.test.ts`：1 file / 3 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/lib/curated-frames/capture.test.ts src/lib/curated-frames/db.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：6 files / 24 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameGrid.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/lib/curated-frames/capture.test.ts src/lib/curated-frames/db.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：7 files / 26 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameActorsTab.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：6 files / 16 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameMoviesTab.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：7 files / 18 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameTimelineTab.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameTimelineTab.test.ts src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：8 files / 20 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameTagFilterBar.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameTagFilterBar.test.ts src/components/jav-library/CuratedFrameTimelineTab.test.ts src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：9 files / 22 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameLibraryToolbar.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameLibraryToolbar.test.ts src/components/jav-library/CuratedFrameTagFilterBar.test.ts src/components/jav-library/CuratedFrameTimelineTab.test.ts src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：10 files / 24 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameDeleteConfirmDialog.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameDeleteConfirmDialog.test.ts src/components/jav-library/CuratedFrameLibraryToolbar.test.ts src/components/jav-library/CuratedFrameTagFilterBar.test.ts src/components/jav-library/CuratedFrameTimelineTab.test.ts src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：11 files / 26 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameEmptyState.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/CuratedFrameEmptyState.test.ts src/components/jav-library/CuratedFrameDeleteConfirmDialog.test.ts src/components/jav-library/CuratedFrameLibraryToolbar.test.ts src/components/jav-library/CuratedFrameTagFilterBar.test.ts src/components/jav-library/CuratedFrameTimelineTab.test.ts src/components/jav-library/CuratedFrameMoviesTab.test.ts src/components/jav-library/CuratedFrameActorsTab.test.ts src/components/jav-library/CuratedFrameGrid.test.ts src/components/jav-library/CuratedFrameCard.test.ts src/lib/curated-frames/selection.test.ts src/components/jav-library/CuratedFrameContextMenu.test.ts src/components/jav-library/CuratedFramesBatchActionBar.test.ts`：12 files / 28 tests passed
...
- `pnpm test -- src/lib/player-playback-stats-format.test.ts`：1 file / 5 tests passed
- `pnpm test -- src/lib/player-playback-stats-format.test.ts src/components/jav-library/PlayerPage.loading.test.ts src/components/jav-library/PlayerPage.i18n.test.ts src/lib/player-playback-clock.test.ts src/lib/player-media-source.test.ts src/lib/player-immersive-chrome.test.ts src/lib/player-shortcuts.test.ts`：7 files / 35 tests passed
- `pnpm test -- src/lib/player-playback-timeline.test.ts`：1 file / 5 tests passed
- `pnpm test -- src/lib/player-playback-timeline.test.ts src/lib/player-playback-stats-format.test.ts src/components/jav-library/PlayerPage.loading.test.ts src/components/jav-library/PlayerPage.i18n.test.ts src/lib/player-playback-clock.test.ts src/lib/player-media-source.test.ts src/lib/player-immersive-chrome.test.ts src/lib/player-shortcuts.test.ts`：8 files / 40 tests passed
- `pnpm test -- src/lib/player-playback-stats.test.ts`：1 file / 5 tests passed
- `pnpm test -- src/lib/player-playback-stats.test.ts src/lib/player-playback-timeline.test.ts src/lib/player-playback-stats-format.test.ts src/components/jav-library/PlayerPage.loading.test.ts src/components/jav-library/PlayerPage.i18n.test.ts src/lib/player-playback-clock.test.ts src/lib/player-media-source.test.ts src/lib/player-immersive-chrome.test.ts src/lib/player-shortcuts.test.ts`：9 files / 45 tests passed
...
- `pnpm test -- src/components/jav-library/DetailPage.test.ts`：1 file / 2 tests passed
- `pnpm test -- src/components/jav-library/PreviewImageViewerInner.test.ts`：1 file / 2 tests passed
...
- `pnpm test -- src/components/jav-library/settings/SettingsMetadataSection.test.ts src/components/jav-library/settings/SettingsMetadataAutomationSection.test.ts src/components/jav-library/settings/SettingsMetadataModeSection.test.ts src/components/jav-library/settings/SettingsMetadataProviderSelectSection.test.ts src/components/jav-library/settings/SettingsMetadataProviderChainSection.test.ts src/components/jav-library/settings/SettingsMetadataTriggerScrapeSection.test.ts`：6 files / 17 tests passed
...
- `pnpm test -- src/components/jav-library/settings/SettingsLibraryPathList.test.ts src/components/jav-library/settings/SettingsLibraryPathToolbar.test.ts src/components/jav-library/settings/SettingsLibraryPathAddDialog.test.ts`：3 files / 9 tests passed
- `pnpm test -- src/components/jav-library/settings/SettingsLibraryPathsSection.test.ts src/components/jav-library/settings/SettingsLibraryPathList.test.ts src/components/jav-library/settings/SettingsLibraryPathToolbar.test.ts src/components/jav-library/settings/SettingsLibraryPathAddDialog.test.ts src/components/jav-library/settings/SettingsLibraryPathRemoveDialog.test.ts`：5 files / 14 tests passed
- `pnpm test -- src/components/jav-library/settings`：22 files / 57 tests passed
- `pnpm test -- src/components/jav-library/settings`：23 files / 60 tests passed
...
### 2.1 SettingsPage 拆分（2028 行）

**状态（2026-05-01）:** 进行中。已抽出 `SettingsOverviewSection.vue`，将 dashboard stats 总览卡片从 `SettingsPage.vue` 中移出；已抽出 `SettingsGeneralSection.vue`，将语言、主题、开机启动与日志设置入口从父组件中移出；已抽出 `SettingsOrganizeSection.vue`，将资料库自动整理卡片从父组件中移出；已抽出 `SettingsNetworkSection.vue`，将出站代理表单、认证折叠、保存与连通性测试按钮从父组件中移出；已抽出 `SettingsMetadataSection.vue`，将元数据 provider 卡片、指定/链模式条件渲染、provider health 匹配、同步/错误反馈与触发刮削入口从父组件中移出；已抽出 `SettingsMetadataAutomationSection.vue`，将 provider 全量健康检查、自动影片刮削与自动演员资料刮削开关从父组件中移出；已抽出 `SettingsMetadataModeSection.vue`，将 metadata provider mode 三选一 fieldset 从父组件中移出；已抽出 `SettingsMetadataProviderSelectSection.vue`，将单 Provider 选择、当前站点 ping 与健康状态展示从父组件中移出；已抽出 `SettingsMetadataProviderChainSection.vue`，将多源 Provider 链列表、健康状态、行级 ping、新增、移除、保存与空态/错误态从父组件中移出；已抽出 `SettingsMetadataTriggerScrapeSection.vue`，将手动触发全库刮削卡片、busy/success/error 展示从父组件中移出；已抽出 `SettingsLibraryPathsSection.vue`，将资料库路径卡片、扫描错误、元数据批量刷新反馈与子组件事件转发从父组件中移出；已抽出 `SettingsLibraryPathToolbar.vue`，将资料库路径批量管理工具条从父组件中移出；已抽出 `SettingsLibraryPathList.vue`，将资料库路径普通行、编辑行、批量勾选与行动作转发从父组件中移出；已抽出 `SettingsLibraryPathAddDialog.vue` 与 `SettingsLibraryPathRemoveDialog.vue`，将资料库路径新增/移除弹窗从父组件中移出；已抽出 `SettingsCuratedSection.vue`，将策展帧保存策略、快捷键入口、导出格式、导出目录与 CORS 提示从父组件中移出；已抽出 `SettingsMaintenanceSection.vue`，将维护工具区从父组件中移出；已抽出 `SettingsAboutSection.vue`，将关于页版本/构建信息、更新状态与首页推荐开发工具从父组件中移出。父组件保留路由 tab、状态计算、`fullScanBusy`、`runFullScan`、后端健康状态计算与 `loadAboutHealth()`，通过 props/emits 对接。回归测试见 `src/components/jav-library/settings/SettingsOverviewSection.test.ts`、`src/components/jav-library/settings/SettingsGeneralSection.test.ts`、`src/components/jav-library/settings/SettingsOrganizeSection.test.ts`、`src/components/jav-library/settings/SettingsNetworkSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataAutomationSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataModeSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataProviderSelectSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataProviderChainSection.test.ts`、`src/components/jav-library/settings/SettingsMetadataTriggerScrapeSection.test.ts`、`src/components/jav-library/settings/SettingsLibraryPathsSection.test.ts`、`src/components/jav-library/settings/SettingsLibraryPathToolbar.test.ts`、`src/components/jav-library/settings/SettingsLibraryPathList.test.ts`、`src/components/jav-library/settings/SettingsLibraryPathAddDialog.test.ts`、`src/components/jav-library/settings/SettingsLibraryPathRemoveDialog.test.ts`、`src/components/jav-library/settings/SettingsCuratedSection.test.ts`、`src/components/jav-library/settings/SettingsMaintenanceSection.test.ts` 与 `src/components/jav-library/settings/SettingsAboutSection.test.ts`。
...
### 2.2 PlayerPage 拆分（2594 行）

**状态（2026-05-01）:** 进行中。已抽出 `src/lib/player-playback-stats-format.ts`，将播放统计面板里的 bitrate / resolution / fps / timecode / percent / count / reason / session kind / source format / transcode profile / unavailable 判定从 `PlayerPage.vue` 中移出，并补 `src/lib/player-playback-stats-format.test.ts` 覆盖缺失值、clamp、profile 映射与格式化边界；已抽出 `src/lib/player-playback-timeline.ts`，将时钟显示、总时长解析、HLS offset、绝对播放时间、进度目标归一化与 seek 目标 clamp 从页面内纯逻辑中移出，并补 `src/lib/player-playback-timeline.test.ts`；已抽出 `src/lib/player-playback-stats.ts`，将 video dimensions、HLS level、bandwidth estimate、fragment stats 平滑与有限数字解析从页面内 reducer 中移出，并补 `src/lib/player-playback-stats.test.ts`。`PlayerPage.vue` 当前仍保留 media element 生命周期、HLS session、进度上报、键盘/沉浸 chrome 与截图编排逻辑。
...
### 2.3 CuratedFramesLibrary 拆分（1475 行）

**状态（2026-05-01）:** 进行中。已抽出 `src/lib/curated-frames/selection.ts`，将批量导出选择的清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推从 `CuratedFramesLibrary.vue` 中移出，并补 `src/lib/curated-frames/selection.test.ts`；已抽出 `src/components/jav-library/CuratedFrameCard.vue`，将重复单帧卡片 UI、批量勾选、近重复 badge、打开 dialog 与右键菜单事件转发收口，并补 `src/components/jav-library/CuratedFrameCard.test.ts`；已抽出 `src/components/jav-library/CuratedFrameGrid.vue`，复用 timeline / actors / movies 三处网格渲染、选择态/近重复态注入和事件转发，并补 `src/components/jav-library/CuratedFrameGrid.test.ts`；已抽出 `src/components/jav-library/CuratedFrameTimelineTab.vue`，将 timeline tab 的 grid 状态注入与事件转发收口，并补 `src/components/jav-library/CuratedFrameTimelineTab.test.ts`；已抽出 `src/components/jav-library/CuratedFrameActorsTab.vue`，将演员分组标题、批量分组勾选和 actor section 事件转发从父组件移出，并补 `src/components/jav-library/CuratedFrameActorsTab.test.ts`；已抽出 `src/components/jav-library/CuratedFrameMoviesTab.vue`，将影片分组标题/副标题、批量分组勾选和影片 tab 事件转发从父组件移出，并补 `src/components/jav-library/CuratedFrameMoviesTab.test.ts`；已抽出 `src/components/jav-library/CuratedFrameTagFilterBar.vue`，将 tag facets、active 状态、展开按钮和 filter 事件转发从父组件移出，并补 `src/components/jav-library/CuratedFrameTagFilterBar.test.ts`；已抽出 `src/components/jav-library/CuratedFrameLibraryToolbar.vue`，将 tab triggers、页数摘要、批量管理入口和批量工具栏按钮从父组件移出，并补 `src/components/jav-library/CuratedFrameLibraryToolbar.test.ts`；已抽出 `src/components/jav-library/CuratedFrameDeleteConfirmDialog.vue`，将删除确认 dialog 渲染与确认事件从父组件移出，并补 `src/components/jav-library/CuratedFrameDeleteConfirmDialog.test.ts`；已抽出 `src/components/jav-library/CuratedFrameEmptyState.vue`，将全库空状态与筛选空状态统一收口，并补 `src/components/jav-library/CuratedFrameEmptyState.test.ts`。组件仍保留分页、dialog tag 编辑和导出/删除 orchestration。
...
**状态（2026-05-01）:** 已完成。`HomeView` 已将 `playbackProgressRevision` 对 `portalModel` 的触发改为 5s 防抖；静态 render hints 已落到 `ScanProgressDock` 统计标签、`DetailPage` 空预览占位块、`LibraryBatchActionBar` 批量确认弹窗静态标题，并由 `src/components/jav-library/render-hints.test.ts` 回归约束。详情页图片加载体验已继续优化：预览网格只让前 4 张 eager，且仅第 1 张 high priority，后续图改为 lazy/low；大图查看器主图按当前图/邻近图 eager、其余 lazy，缩略图 lazy/low，并在主图 load 前显示占位与淡入，避免打开查看器时全量并发加载和突兀闪现。
