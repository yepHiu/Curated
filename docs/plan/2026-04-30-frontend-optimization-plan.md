...
| 2.3 CuratedFramesLibrary 拆分 | 进行中 | 已抽出批量导出选择 reducer `curated-frames/selection`，覆盖清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截；组件继续负责 toast/i18n 与视图分组 | `src/components/jav-library/CuratedFramesLibrary.vue`, `src/lib/curated-frames/selection.ts`, `src/lib/curated-frames/selection.test.ts` |
...
### 2.3 CuratedFramesLibrary 拆分（1826 行）

**状态（2026-05-01）:** 进行中。已抽出 `src/lib/curated-frames/selection.ts`，将批量导出选择的清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截从 `CuratedFramesLibrary.vue` 中移出，并补 `src/lib/curated-frames/selection.test.ts`。组件仍保留分组、分页、dialog tag 编辑、导出/删除 orchestration 与 tab UI。
