...
| 2.2 PlayerPage 拆分 | 进行中 | 已先抽出播放统计格式化 helper `player-playback-stats-format`，覆盖 bitrate、resolution、fps、timecode、percent、count、reason、session kind、source format、transcode profile 与 unavailable 判定；`PlayerPage.vue` 保留播放状态编排和 UI 引用 | `src/components/jav-library/PlayerPage.vue`, `src/lib/player-playback-stats-format.ts`, `src/lib/player-playback-stats-format.test.ts` |
