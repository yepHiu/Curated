...
| 2.3 CuratedFramesLibrary 拆分 | 进行中 | 已抽出批量导出选择 reducer `curated-frames/selection`，覆盖清空、单选/取消、20 个上限、按演员命名桶与匿名桶混选拦截，以及取消演员分组勾选后的导出桶反推；已抽出单帧卡片 `CuratedFrameCard`、三视图共用网格 `CuratedFrameGrid` 与演员 tab `CuratedFrameActorsTab`，复用 timeline / actors / movies 三处卡片布局、选择态、近重复态和事件转发，并收口演员分组头渲染/勾选事件；组件继续负责 toast/i18n 与视图分组数据 | `src/components/jav-library/CuratedFramesLibrary.vue`, `src/components/jav-library/CuratedFrameCard.vue`, `src/components/jav-library/CuratedFrameCard.test.ts`, `src/components/jav-library/CuratedFrameGrid.vue`, `src/components/jav-library/CuratedFrameGrid.test.ts`, `src/components/jav-library/CuratedFrameActorsTab.vue`, `src/components/jav-library/CuratedFrameActorsTab.test.ts`, `src/lib/curated-frames/selection.ts`, `src/lib/curated-frames/selection.test.ts` |
