...
| 阶段 2：增加自动化显示缩放冒烟测试 | 已完成 | 已新增 Chrome + Firefox 测试矩阵；`pnpm test:display` 通过 108 个用例 |
| 阶段 3：增加共享显示适配 tokens | 暂停 | 多端适配工作量较大，暂不展开全局 token 改造 |
| 阶段 4：AppShell 布局稳定化 | 暂停 | 暂不做跨设备壳层重构，后续按桌面 Web 具体问题局部处理 |
| 阶段 5：设置页适配 | 暂停 | 暂不做完整多端设置页适配，后续按桌面 Web 问题局部处理 |
| 阶段 6：播放器 HUD 适配 | 暂停 | 暂不做全矩阵播放器 HUD 适配，后续按桌面 Web 问题局部处理 |
| 阶段 7：网格、卡片、标签适配 | 暂停 | 暂不做全量网格/卡片缩放治理，后续按桌面 Web 问题局部处理 |
| 阶段 8：对话框和 overlay 适配 | 暂停 | 暂不做全量 overlay 适配，后续按桌面 Web 问题局部处理 |

## 阶段性结论

多端屏幕、分辨率、DPR、系统缩放和浏览器缩放的完整适配是一项横跨壳层、网格、播放器、设置页、对话框、全局 tokens 与自动化截图回归的系统工程。当前阶段先在阶段 1 和阶段 2 画句号：

- 保留显示缩放检查清单，作为后续 UI 变更的审查参考。
- 保留 `pnpm test:display`，作为桌面 Web 端 Chrome + Firefox 的基础回归护栏。
- `pnpm test:display` 和其他 display scaling 冒烟测试耗时较长，后续未经用户明确同意不得主动运行。
- 暂停阶段 3 到阶段 8 的全量多端适配改造。
- 后续优先聚焦桌面 Web 端的真实问题，按具体页面和具体症状做小步修复，不开启大范围跨端响应式重构。

后续如果重新启动多端适配，应先重新确认产品范围、目标设备矩阵、验收标准和可接受的 UI 改动幅度，再继续执行阶段 3 及之后的任务。
...
- 使用 Playwright Chrome + Firefox；Windows 下不测试 WebKit。
...
- `pnpm test:display` 已存在，Windows 下覆盖 Chrome + Firefox 的核心页面冒烟测试。
