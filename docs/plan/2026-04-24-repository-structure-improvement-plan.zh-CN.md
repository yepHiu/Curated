...
状态：proposed（原目录重排方案；下列 GitHub 跟踪清理为独立已执行范围）

## 2026-09-25：GitHub 跟踪范围精简

用户明确要求保留全部文档和原型。本次不执行下文的源码目录迁移或文档重排，仅移除误入版本控制的本地产物：

- `backend/frontend-dist/`：50 个旧前端构建文件，由源码构建和发布流程生成。
- `.claude/settings.local.json`：1 个本机权限配置。
- `.agents/`：3 个已在忽略规则中的可选本地工具文件。

共 54 个文件、1,472,985 字节，只取消 Git 跟踪，当前本机文件原位保留；已生成安装包与便携包不删除。约 24.65 MB 的 HarmonyOS 字体仍被正式界面引用，保留字体及其许可证。全部 Markdown、HTML 原型及其资源、项目看板、共享 `.cursor/` 规则与技能继续跟踪。

补充忽略根 `frontend-dist/`、FFmpeg 本地二进制目录、`output/`、覆盖率与 Playwright 报告；新增 CI 检查，禁止被忽略文件继续留在 Git 索引。完整规则见 [手册](../guide.md#repository-tracking-policy)。本次没有重写历史，也不自动推送远程，因此不会立即缩小历史克隆体积或删除 GitHub 上既有版本。

验收通过：索引中无已忽略文件；54 个本地副本 SHA-256 均未变化；11 个本地产物路径被忽略、12 个必需源码/文档路径未被误忽略；CI 在独立临时索引中接受正常状态、拒绝强制添加的构建产物；`cd backend && go test ./internal/webui` 通过；`git diff --check` 通过。未改动应用源码，未运行前端全量构建。原方案其余阶段仍为历史建议，不表示已经批准实施。
