...
| `1.25` | 系统缩放近似 |
| `1.5` | Windows 常见缩放近似；应保持默认桌面密度，不触发 Retina 紧凑变量 |
| `2` | macOS Retina；桌面 hover/fine pointer 且宽度 ≥ `1024px` 时触发紧凑密度变量 |
| `3` | 高密移动设备 |
...
`pnpm test:display` 和其他 display scaling 冒烟测试耗时较长，代理不得在没有用户明确同意的情况下主动运行。需要运行时，应先说明预计影响，并获得用户确认。
