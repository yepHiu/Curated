...
| 字段 | 说明 |
|------|------|
| `aiProvider` | OpenAI 兼容模型的 kind/baseUrl/apiKey/model/contextWindow；contextWindow 为总上下文 tokens，省略或旧配置 0 默认 65536，界面与 PATCH 接受 32768–2097152 整数，下一聊天请求生效；经 `GET/PATCH /api/settings` 保存，密钥不进入发布样例。 |
