...
影片、收藏/最近加入/回收站、演员及演员影片、漫画、写真、愿望单、首页、萃取帧、观看历史统一使用领域组件 `MediaEmptyState`，由 `components/ui/empty` 组合，保留 AppShell 内部滚动。居中图标、18px 标题、14px 简短提示；同一虚线边框与语义弱底色，无动作时不留按钮空行。

- 内容为空：统一标题「目前这里空空如也」，按页面补一句面向用户的提示，不展示 Mock、压缩包格式、扫描流程等实现说明。
- 搜索/筛选无结果：统一「没有找到匹配的内容」与调整关键词/筛选提示，已有清除筛选动作继续使用；排序不算筛选。
- 加载与失败独立呈现，不同时宣布资料库为空；已有内容在刷新失败时可以继续展示。
- 愿望单默认待入库页为空时，读取一次全部状态的轻量计数，避免已有已入库/已完成条目时误报整个集合为空。
- 动作用 outline 圆角按钮，窄屏至少 44px；不额外增加无权限的管理入口。中文、英文、日文语义同步。
