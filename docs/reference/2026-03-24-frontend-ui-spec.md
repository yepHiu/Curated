...
萃取帧（2026-09-06）：截图回执属于播放面局部反馈，使用 `CaptureReceipt` 缩略图、时间与状态；控件事件阻止传播至视频，触屏主要动作至少 44px。正常成功回执 2 秒后退出，失败候选最多保留 30 秒供用户重试。大图使用 `FrameImageViewer`，支持尺寸、加载失败重试、100% 与适应窗口。全屏播放器的 Dialog / DropdownMenu 可传 `portalTo` 定位到全屏 surface；其它页面默认行为不变。图库离屏卡片行保留布局高度和可聚焦加载入口，当前焦点所在行不卸载。
...
- `/wishlist` 使用 AppShell 标准页面间距，标题/待入库计数、状态 Tabs、搜索 Input、按行虚拟网格与游标分页。卡片复用 MediaStill，以缩略图和番号为主，不出现播放按钮、文件规格或播放进度。
- `/wishlist/:id` 提供独立资料详情、缺图占位、补全状态、备注与番号修正、完成/恢复、重试、删除确认以及已关联影片跳转。详情使用自己的「返回愿望单」，AppShell 不再显示返回资料库。
- 现有 Button/Input/Badge/Field/Dialog/Tabs/Empty/Skeleton 组件，不新增平行基础 UI。中文/英文/日文文案对齐。
- 状态失败保留条目；Mock 空态明确不支持真实插件连接。网络页使用标准设置卡片和带标签的 Switch 控制插件联动，无凭证 UI；保存中禁用，失败保留原值并允许重试。愿望单联动关闭时用灰色状态点和“联动已关闭”提示。

## 个人洞察页面（2026-09-24）

- `/insights` 是 AppShell 内的个人数据阅读面。时间范围在紧凑控件中切换，实际日期、日界线与数据起始日就近显示；手机端四个选项为 2×2，触控区域至少 44px。
- 六项摘要在桌面为 3×2、手机为 2 列。观看时长、开始观看、完成率是第一层；当前完成数、已评分数、平均评分是第二层。卡面保留简短指标提示。
- 演员、厂牌、标签排行维持各自 Card 与条形列表，默认展示前五项，更多条目由该卡片的按钮展开。百分比仍按 `full-per-entity` 计算；页面不展示额外归因说明。
- 切换范围时可暂留上一范围的摘要，必须显式标注旧范围；新摘要到达后各排行独立加载与重试。事实摘要仅由满足样本门槛的聚合数值生成，不推断未提供的趋势。

## macOS 融合窗口栏（2026-09-26）

仅在 preload 提供 `windowChrome: "macos"` 时启用：原生红黄绿按钮位于侧栏独立的 40px 顶部区域，品牌标题位于其下方，品牌与导航之间不画分割线。宽窗口右侧工具栏从窗口顶部开始，沿用原有高度，不增加 40px 留白。顶部区域延续侧栏背景；折叠侧栏保留品牌图案，窄窗口隐藏侧栏后，内容区及抽屉仍预留顶部空间以避让按钮。顶部空白区域可拖动，交互控件区域使用 `no-drag`。主题沿用 sidebar/background 令牌；Web 与其它桌面平台保持原样。阅读器与独立锁屏/故障页保留顶部拖动区域。

## 开发调试入口（2026-09-26）

开发态壳层在 DEV / PERF 标签旁提供 DEBUG 按钮，打开页面内调试 Dialog；调试操作、性能监控、日志以 Tabs 分区。新的开发触发项优先加入此处，常规扫描、备份等维护入口不迁入。正式环境不显示 Debug，日志配置继续在设置页。

复用 Dialog、Tabs、SettingsHint、SettingsScopeBadge，短标题、语义色、限高内部滚动；普通说明在标题下方 Tooltip。读取失败提供重试，未初始化配置不可写入，执行中禁止重复提交，结果就地反馈。弹窗期间性能条隐藏，关闭后焦点回到入口；性能采样复用原实例，避免重复订阅。
