...
- 六项摘要在桌面为 3×2、手机为 2 列。观看时长、开始观看、完成率是第一层；当前完成数、已评分数、平均评分是第二层。卡面用简短提示，完整口径放在可展开的「统计口径」内。
- 演员、厂牌、标签排行维持各自 Card 与条形列表，默认展示前五项，更多条目由该卡片的按钮展开。百分比仍按 `full-per-entity` 计算，页内说明跨实体合计可能超过 100%。
