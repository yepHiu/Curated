...
仅在 preload 提供 `windowChrome: "macos"` 时启用：原生红黄绿按钮位于独立的 40px 顶部区域，品牌标题与内容工具栏位于下一行，沿用原有高度与侧栏宽度。顶部区域延续两栏背景，不增加色块或横向分隔线；折叠侧栏保留品牌图案，窄窗口及抽屉同样预留顶部空间。顶部空白区域可拖动，交互控件区域使用 `no-drag`。主题沿用 sidebar/background 令牌；Web 与其它桌面平台保持原样。阅读器与独立锁屏/故障页保留顶部拖动区域。
