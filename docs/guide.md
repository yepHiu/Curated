...
Only the publish job has `contents:write`. It verifies uploaded GitHub digests, publishes the release, then advances component manifests with a fast-forward Git Data API update. If channel advancement fails after publication, download the preserved `component-release-<tag>-<attempt>` Actions artifact into `.workspace/component-cd`, check out the exact tag, and run `python scripts/release/component_cd.py channels --tag <tag>` with `GH_TOKEN` and `GITHUB_REPOSITORY` configured. This recovery verifies public assets and never rebuilds or overwrites the release. Normal failed draft builds may be retried from the same immutable tag.
