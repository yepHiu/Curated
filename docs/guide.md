...
库页工具栏显示册数，排序通过右侧菜单选择；搜索条件可直接清除，加载失败可以重新加载。详情封面可直接打开第一页；主按钮显示浏览或从上次页码继续，源文件名、页数和添加日期集中展示，长存储路径按需展开。写真当前只展示已有浏览能力，不显示尚未接通的编辑、删除、打开源文件操作。

图片预览根据实际容器宽度展示两行，窄屏通常 4 张，桌面自动增加，最多 20 张。使用「上一批 / 下一批」查看全书，输入页码后可「定位」预览或「打开此页」进入阅读器。所选页有边框提示；横图保留完整画面。每批替换旧节点，只加载接近可见区域的图片，失败时点击重试。

写真预览与封面现在使用缩略图，阅读器仍读取原图。过大的或不可解码的图片可能无法生成预览，但不会自动改为下载大原图；可通过「打开此页」浏览原图。
