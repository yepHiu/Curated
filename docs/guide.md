...
The split distribution is built by `.github/workflows/cd-release.yml` (**CD - Full, Server and Desktop**). Use `full-v1.6.0` for the initial complete release, with Server 1.6.0 and Desktop 0.1.0. Subsequent `server-vX.Y.Z` / `desktop-vX.Y.Z` tags release only that component. Source versions live in `backend/internal/version/server.json` and `scripts/release/versions/{desktop,full}.json`. Each tag needs `docs/release-notes/<tag>.md`; CD never increments versions or moves tags.

| Download | Contents |
|---|---|
| `Curated-Full-Setup-1.6.0-windows-x64.exe` | Offline bootstrapper containing the exact Server and Desktop standalone installers |
| `Curated-Server-Setup-1.6.0-windows-x64.exe` | Go Server, complete Web UI and FFmpeg; no Electron |
| `Curated-Desktop-Setup-0.1.0-windows-x64.exe` | Electron client and local connections; no Server or business Web UI |
| `Curated-<Component>-<version>-windows-x64.zip` | Standalone runtime for Server/Desktop; offline installer kit for Full |
| `Curated-Desktop-0.1.0-macos-arm64.dmg` / `.zip` | Apple Silicon Desktop only; ad-hoc signed, not Apple notarized |

Full runs the independent installers with fixed identities `Curated.Server` and `Curated.Desktop`, per-user directories `%LOCALAPPDATA%\Programs\Curated\{Server,Desktop}`, and separate uninstall entries. Full has no additional uninstall entry. Existing equal/newer components are reused; standalone installers reject downgrade. A partial failure reports the failed component, returns nonzero and keeps successful/existing components. Retry Full after resolving the failure. Program uninstallation preserves Server data and Desktop connections.

Server runs independently in the signed-in user's tray; its own settings control login startup. Launch **Curated Server** to manage it; use **Curated Desktop** for the client. Exiting Desktop leaves Server running. Full starts Server, then an empty Desktop profile reads an account-local address hint written after Server binds and probes that actual address. Existing connections are not replaced. If startup is slow or the Server cannot start, the local connection page remains available for retry/manual connection. This is not a Windows system service.

CD first runs CI on the immutable tag commit. The Windows runner compiles installers and checks clean installation, Full reuse/retry, downgrade rejection, program uninstall/data preservation, Server Web serving and packaged Desktop startup. Full also tests connecting Desktop and verifies Server survives Desktop exit. A public release requires every requested platform and every package/hash to pass. Manual existing-tag runs default to draft. Already public releases and production packages are never overwritten. Same-version component artifacts are downloaded and checksum-verified from their published manifest for reuse; a changed component requires a new version.

**Update isolation:** old clients still read `/releases/latest`, so that endpoint remains pinned to the compatible legacy all-in-one release. New stable component releases explicitly use `make_latest=false`; CD validates the old latest before/after publication and returns the new release to draft if isolation fails. New downloads should use [all releases](https://github.com/yepHiu/Curated/releases), not the legacy Latest shortcut. Component manifests on the `release-channels` branch provide `component/variant/channel/version/platform/arch/format/url/sha256`; Full records exact component versions. Server accepts only the matching Server EXE and discards legacy cached installer state. Desktop checks its own pinned manifest from the local connection window even without a Server; its first implementation opens the exact official download, without silently installing or verifying the downloaded file. A local standalone Desktop may update a local standalone Server; remote Server update writes remain blocked.

Only the publish job has `contents:write`. It verifies uploaded GitHub digests, publishes the release, then advances component manifests with a fast-forward Git Data API update. The initial channel branch is an orphan commit containing only manifests, so it does not require workflow-writing permission. If channel advancement fails after publication, the recovery workflow recognizes the public release and updates only channels; it never modifies public assets. Alternatively, download the preserved `component-release-<tag>-<attempt>` Actions artifact into `.workspace/component-cd`, check out the exact tag, and run `python scripts/release/component_cd.py channels --tag <tag>` with `GH_TOKEN` and `GITHUB_REPOSITORY` configured. This recovery verifies public assets and never rebuilds or overwrites the release. Normal failed draft builds may be retried from the same immutable tag. If the build gates succeeded but final publication failed, **CD - Recover verified release** accepts the original failed run ID and draft/publish mode, checks those gates again, downloads the original artifacts, and uses the exact tag checkout for versions/notes. It does not rebuild or move the tag. A committed `scripts/release/recovery-request.json` can trigger the same explicit recovery on master; unrelated pushes do not retry it. A GitHub `Resource not accessible by integration` rejection is a publishing-permission failure, not evidence of invalid packages.

The legacy workflow moved to `.github/workflows/cd-legacy.yml`; `vX.Y.Z`, `scripts/release/version.json`, `pnpm release:publish` and old release notes remain for compatible all-in-one maintenance. Historical tags keep their original workflow. The new component workflow does not call the legacy packager.

### Migrating an old all-in-one installation

Full **1.7.2** adds a Windows migration helper. It runs the old uninstaller internally after a verified backup, then installs Server 1.7.2 and Desktop 0.2.0. You do not need to manually uninstall first. This source change still requires the Windows CD installation gates before publication; the already published Full 1.7.1 retains its manual migration requirement.

1. Sign in as the same Windows user that owns the original library. Fully quit Curated from its tray. Run the Full EXE normally; do not use “Run as different user”. Only removal of an old machine-wide program may request UAC; the Full process and new components remain under the original user.
2. When Full finds the old uninstall identity, confirm the original data root (the directory containing `data` and `config`). It defaults to `CURATED_DATA_DIR`, or `%LOCALAPPDATA%\Curated`. If the old Server used `-config`, select that exact JSON runtime configuration on the next page. A missing database is an error, never a reason to create a new empty library.
3. Full locks the original database and creates a verified `.curated-backup` under `%LOCALAPPDATA%\Curated\installer-migrations\backup-*`. It also saves custom runtime configuration and an existing Server identity file. It does not change the live database schema during backup. Media stays at its original location and is not included in the backup archive.
4. After backup, Full invokes the registered old uninstaller, replaces the old Server login entry when appropriate, and installs both independent components. Partial failure returns an error; keep the journal/backups and run Full again. Cancelling UAC leaves the old startup entry intact; retrying takes a fresh backup. The old program may already have been removed if a later component fails, so retry Full rather than deleting the migration records.
5. Confirm your original library/settings and Desktop connection after installation. Saved `servers.json` is copied from the old `curated-desktop` profile only if the new `Curated Desktop` profile has no connection file. Original profiles remain. Chromium authentication/session databases are not merged; unlock again if prompted.

**Persistent custom startup:** `%LOCALAPPDATA%\Curated\server-startup.json` records the original data root, database and optional saved runtime configuration. Standalone Server loads it for ordinary shortcuts and login startup. An explicit `-config` or a different `CURATED_DATA_DIR` overrides it. A matching inherited data-root environment still uses the saved custom configuration. Do not delete the migration backup directory: a migrated custom configuration is retained there and referenced by the startup profile. To change the active custom configuration later, use its recorded `configPath` or launch explicitly with `-config`. Missing/corrupt profiles or missing referenced files stop automatic migrated startup instead of creating an empty database.

**Supported layouts and limits:** the automatic path recognizes the registered 1.0–1.5 all-in-one identity, including non-default program directories. Data, cache/log destinations and registered media roots must be absolute and outside the old program directory; linked program trees and program-local database/library configuration are blocked before removal. Simultaneous legacy and split Server installations, multiple legacy registrations and uninspectable/running Curated processes are also blocked. A portable installation, another account's data, relative-path configuration or a nonstandard old uninstall command still requires the explicit migration procedure below. Recognition of the identity is not a claim that every historical package has passed testing: CD pins real 1.4.13 and 1.5.8 packages and checks their hashes and actual upgrades. Split 1.6/1.7 upgrades keep their existing component identities and never downgrade a newer component.

For unattended same-user installations, supply `/LEGACYDATADIR="D:\CuratedData"` and, if required, `/LEGACYCONFIG="D:\Config\server.json"`. `/NOLAUNCH=1` suppresses final app launch but still performs backup/migration. It does not suppress required Windows UAC. Old update clients keep their legacy Latest feed; download and run Full explicitly.

**Nonstandard layouts / recovery:** create and verify a backup with the old app, record its data root/configuration and turn off old login startup. Fully exit it, relocate any data inside its program directory using the documented path-migration workflow, then retry Full or uninstall only the old program and install the components as the same user. Preserve `%LOCALAPPDATA%\Curated`, external data, media and backups. Never delete runtime lock files to bypass a running process. After a newer Server has opened/migrated the database, restoring the old application requires the verified pre-upgrade database/configuration backup; never point an old Server directly at that newer database. Automatic program rollback after schema migration is not provided.

On Apple Silicon macOS, `pnpm release:macos-desktop` produces DMG/ZIP in `release/macos-desktop/`. Use `python3 scripts/release/release_cli.py package-macos-desktop --output-dir <new-directory>` for another validation build; existing files are not overwritten. Windows component builds use `python scripts/release/component_cd.py windows --tag <existing-component-tag>` from the exact tag checkout, with `VITE_USE_WEB_API=true`, Go, Inno Setup, FFmpeg and the locked Electron runtime installed.
...
- Open **服务器 / Servers** in the tray; macOS also provides an application-menu entry. Older Server UIs can use the tray even without the settings card. The bundled management window works while disconnected or PIN-locked.
...
- Edit names/addresses or delete saved records. The current connection can be renamed, but switch away before changing its address or deleting it. Removing a record never deletes Server data.
...
## Desktop discovery and upgrade (Full/Server 1.7.0, Desktop 0.2.0)

Desktop now uses a bundled Vue connection window with recent connections, IPv4 SSDP discovery, local theme, system/direct/manual proxy and OS login startup. It never starts a Server in development or standalone distributions. `pnpm dev:electron` builds the launcher and opens a branded desktop app; run the Server separately. `pnpm build:electron:main` also builds the launcher assets. The packaged legacy distribution retains its old backend lifecycle.

`GET /api/server-info` is public even while PIN-locked and returns the persistent Server UUID, product version and protocol/bridge version 1. `<databasePath>.server-id` stores the UUID separately from SQLite. Preserve it when moving the same Server; a corrupt identity is an explicit startup error, not silently replaced. Discovery exposes the Server name and address to the local network. `discoveryEnabled` defaults true but only advertises when LAN access is enabled and listening; changing it requires a Server restart. `/discovery/description.xml` is only available with discovery enabled. UDP multicast may be blocked by firewalls/VLANs; manual HTTP/HTTPS root-address connections still work.

Released Desktop `userData/servers.json`, saved IDs/names/last connection and existing origin partitions are preserved. First UUID binding, including a previously recorded legacy Server, retains the existing session. A later UUID change requires confirmation and uses an isolated identity partition. Initial binding cannot prove the old endpoint was not already replaced. Development-only `connections.json` is imported only if `servers.json` is absent, validated completely before writing and retained as a backup. Corrupt configuration is not silently reset. Desktop 0.2.0 supports older health-only Servers, including PIN-locked 1.5.x/1.6.x; unsupported future protocols fail explicitly. Old Desktop's health endpoint and preload API remain available.

Desktop device settings live only in `userData/desktop-settings.json`; proxy changes require Desktop restart and never change Server proxy credentials. Settings → Network on the Server retains its existing local-only administrative restrictions. Native server actions also require a direct local request. A split 1.6.0 install upgrades using the same component IDs and install locations; no new SQLite migration is added here. Back up and verify before upgrading. Legacy all-in-one ≤1.5.8 still requires the manual migration in §8; cross-account/custom-config migration is not automatic. Preserve the original user/data/config/port. Mac is Apple Silicon only and remains ad-hoc signed, without Apple notarization.

CD uses independent versions and immutable component tags. `package.yml` delegates to the canonical CD with draft mode, so manual builds have the same names and checks. Full provides Windows Full/Server/Desktop EXE+ZIP and macOS Desktop DMG+ZIP. Windows smoke installs the pinned previously published 1.6.0/0.1.0 EXEs, checks their SHA-256, upgrades and validates install identities and a real old database before exercising clean install/lifecycle checks. Real Desktop upgrade smoke verifies records, localStorage and cookies across the old/new shells and Server identity transition.
