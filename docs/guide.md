...
The wishlist uses the movie library's poster grid, card sizing and compact filter menu. Its browser plugin indicator is read-only. It reads the existing connected-clients API every 15 seconds while visible and reports a connection when a `Curated-Plugin` request was seen within the last five minutes. This is recent activity, not a persistent socket or proof of a valid wishlist token. Failed status requests show unknown; Mock mode shows demo mode. Restart the updated backend to enable separate plugin identification; credential management remains in Settings → Network.
