...
The Server installer detects the old Curated uninstall identity and refuses overlapping installation. This first split release uses an explicit migration, not a silent account/layout conversion:

1. Use the old app's backup flow and verify the backup. Record the actual library/data location, `CURATED_DATA_DIR`, custom `-config` arguments, and configured port. Keep the old installer for recovery.
2. Turn off the old login startup, fully exit the old shell and Server, and uninstall the old program. Keep `%LOCALAPPDATA%\Curated`, media directories and backups. Do not delete runtime lock files to force migration.
3. Run Full/Server as the same normal signed-in user. The default data root stays `%LOCALAPPDATA%\Curated`; configure custom environment/config paths before first launch. For a custom runtime configuration, use `/NOLAUNCH=1`, then launch Server with the original `-config` path before opening Desktop. Do not use another administrator account.
4. Confirm the original library and settings in the browser, then connect Desktop to that Server. Old browser/session storage is not copied to a new origin/profile; unlock again if prompted. Re-enable login startup from Server settings after verification.
5. To roll back, exit both components and follow the verified backup restore procedure. Do not run an old Server against a database already migrated by a newer version.

The default data-root path is preserved; automated migration of arbitrary old program-local configuration, elevated-account data or authentication sessions is not implemented. Windows installation acceptance is a CD gate, and must not be inferred from local macOS tests.
