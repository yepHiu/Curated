...
图片预览根据实际容器宽度展示两行，窄屏通常 4 张，桌面自动增加，最多 20 张。使用「上一批 / 下一批」及数字分页查看全书；数字代表预览批次，当前批次高亮，批次较多时显示省略号并保留首末批入口。点击缩略图进入对应页阅读；不再显示页码输入框、定位或打开此页按钮。横图保留完整画面。每批替换旧节点，只加载接近可见区域的图片，失败时点击重试。
