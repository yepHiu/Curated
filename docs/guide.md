...
Automated Windows and Apple Silicon Desktop CD is defined in `.github/workflows/cd-release.yml`. After the workflow is pushed to the default branch, pushing a canonical `vMAJOR.MINOR.PATCH` tag publishes a release after checks pass. The tag must contain the workflow/scripts, matching `scripts/release/version.json`, and exactly one versioned release note with a nonempty `## GitHub Release Body`. CI runs again against the exact tag commit before Windows packaging. Ordinary branch pushes only run CI.

For recovery of an existing unpublished tag, use Actions → **CD - Windows and Mac Desktop** → **Run workflow**, enter the tag, and choose `draft` (default) or `publish`. A tag push starts a public release; a later manual draft run cannot turn that run into a preview. Published releases cannot be overwritten. Failed drafts may be retried only for the same commit. Fix source/notes with a new version/tag rather than moving an existing tag.

CD uses the built-in GitHub token; only the upload job has release write permission. It verifies both Windows packages, publishes `release.json` and `SHA256SUMS.txt`, and checks uploaded digests before making the release public. Packages remain in Actions artifacts for 14 days; the runner's CSV ledger and bundle reports remain for 90 days and are not committed back automatically. The same release also includes standalone Mac Desktop DMG/ZIP for Apple Silicon, using the independent Desktop version. Windows standalone Desktop, Server/Full split packages, and Intel Mac builds remain outside this workflow. See the [CD implementation and recovery instructions](plan/2026-03-31-production-packaging-and-config-strategy.md).

The macOS job uses `macos-15` (arm64) and packages only Electron, the local server connection window, icons and licenses. First launch asks you to add an existing Curated Server. It does not bundle a Server, FFmpeg or the server-hosted Vue frontend. Packages are ad-hoc signed for arm64 execution, without Developer ID signing or Apple notarization. DMG/ZIP hashes are included in the combined `SHA256SUMS.txt`; `desktop-macos.json` records the component version and source commit. Both platform builds must succeed before publication. Tags that predate this change, including v1.5.8, keep their original workflow.

On Apple Silicon macOS, run `pnpm release:macos-desktop` to build locally into `release/macos-desktop/`. Existing packages are never overwritten; use `python3 scripts/release/release_cli.py package-macos-desktop --output-dir <new-directory>` for another validation build. Desktop versions come from `scripts/release/versions/desktop.json`; the packager does not increment them or change the development distribution. Independent Desktop update feeds remain unconfigured.
