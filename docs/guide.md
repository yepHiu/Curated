...
- Connections load the Server-hosted business UI. Run `pnpm build:electron:main` to include local assets, then restart Desktop; rebuild Server Web UI for the settings card. This change does not deliver SSDP, independent installers, automatic Desktop installation, or the final Server lifecycle split. Legacy-owned local processes still follow legacy quit behavior.
