...
On a wishlist detail page, click **查询播放信息** to check Jable, MISSAV, 123av and Supjav for the saved code. The request runs through `GET /api/wishlist/items/{id}/playback` with the normal application session. A confirmed match displays a link to the external watch page; a missing match displays **未找到**, while blocked sites, timeouts, redirects to another host and changed page layouts display **暂时无法确认**. Results are queried on demand and are not saved to the wishlist. Mock mode requires a connected Curated backend for this feature.
