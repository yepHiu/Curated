...
- A docked Agent panel with record-backed movie answers, server-side fact rendering and checks before publication; AI settings offer optional tool-call limits (unlimited by default), explicit write confirmation, measured token usage, timing and audit records; see the [handbook](docs/guide.md#ai-settings-and-governance).
