...
- A docked Agent panel with automatic context compaction, persistent conversation summaries and record-backed movie answers; AI settings offer configurable model context windows with capacity presets, optional tool-call limits (unlimited by default), explicit write confirmation, measured token usage, timing and audit records; see the [handbook](docs/guide.md#ai-settings-and-governance).
...
Backup, restore, path migration, configuration keys, and release packaging are documented in [docs/guide.md](docs/guide.md). CD builds Windows packages and Apple Silicon Mac Desktop, with version-tag releases and manual draft runs; see [release operations](docs/guide.md#8-release-and-packaging).
