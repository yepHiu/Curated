...
watch(() => props.src, () => { loaded.value = false; failed.value = false; attempt.value = 0; aspectRatio.value = 2 / 3 })
...
<template>
  <button ref="target" type="button" data-book-preview-tile
    ...
    class="group relative max-w-full min-w-0 shrink-0 cursor-pointer overflow-hidden rounded-[1rem] border border-border/70 bg-muted/30 p-0 text-left [--preview-height:10rem] sm:[--preview-height:11rem] xl:[--preview-height:12rem] transition-[border-color,box-shadow] hover:border-primary/50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
    :style="{ width: `calc(var(--preview-height) * ${aspectRatio})`, aspectRatio: String(aspectRatio) }"
    ...
    <img v-if="visible && src && !failed" :key="attempt" :src="src" :alt="label" loading="lazy" decoding="async" fetchpriority="low"
      class="absolute inset-0 size-full object-cover transition-opacity motion-reduce:transition-none" :class="loaded ? 'opacity-100' : 'opacity-0'" @load="onImageLoad" @error="failed = true" />
