...
describe("SettingsPhotoLibrarySection", () => {
  ...
  it("enables photo beta before paths are configured", async () => {
    const wrapper = mount(SettingsPhotoLibrarySection)

    await wrapper.get("[data-photo-beta-switch]").trigger("click")

    expect(wrapper.text()).not.toContain("settings.photoLibraryPathRequired")
