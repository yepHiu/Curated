...
describe("SettingsComicLibrarySection", () => {
  ...
  it("enables comic beta before paths are configured", async () => {
    const wrapper = mount(SettingsComicLibrarySection)

    await wrapper.get("[data-comic-beta-switch]").trigger("click")

    expect(wrapper.text()).not.toContain("settings.comicLibraryPathRequired")
    expect(mockState.comicService?.setComicLibraryEnabled).toHaveBeenCalledWith(true)
  })
