...
/** 持久化漫画 Beta 开关；允许先启用再配置资料库路径。 */
async function setComicLibraryEnabled(value: boolean) {
  if (enableBusy.value) return
  enableError.value = ""
  try {
    enableBusy.value = true
    await comicService.setComicLibraryEnabled(value)
  } catch (err) {
    enableError.value = errorMessage(err)
  } finally {
    ...
    <Card class="gap-2 rounded-xl border border-border bg-card shadow-sm">
      <CardHeader class="grid grid-cols-[auto_minmax(0,1fr)] items-center gap-x-2.5 gap-y-1 pb-0">
        ...
        <CardTitle class="min-w-0 text-lg tracking-tight">
          {{ t("settings.comicLibraryTitle") }} <Badge variant="secondary">Beta</Badge>
        </CardTitle>
        <CardDescription
