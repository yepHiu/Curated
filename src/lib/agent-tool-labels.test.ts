...
describe("agent tool labels", () => {
  it("hides present_movies from the process list", () => {
    ...
    expect(isAgentProcessTool("present_comics")).toBe(false)
    expect(isAgentProcessTool("present_photos")).toBe(false)
    expect(isAgentProcessTool("search_movies")).toBe(true)
    expect(isAgentProcessTool("search_comics")).toBe(true)
  ...
  it("maps known tools to product copy keys", () => {
    ...
    expect(agentToolI18nKey("search_comics")).toBe("agentWindow.tools.searchComics")
    expect(agentToolI18nKey("save_comic_comment")).toBe("agentWindow.tools.saveComment")
    ...
    expect(agentToolI18nKey("update_comic_title")).toBe("agentWindow.tools.updateDisplay")
    expect(agentToolI18nKey("update_photo_title")).toBe("agentWindow.tools.updateDisplay")
