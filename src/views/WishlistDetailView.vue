  ...
  <section class="min-h-0 flex-1 overflow-y-auto">
    ...
    <div v-if="item" class="grid gap-6 lg:grid-cols-[minmax(220px,340px)_1fr]">
      ...
      <div class="flex min-w-0 flex-col gap-4">
        ...
        <div class="flex flex-wrap gap-2"><Button v-for="movieId in item.movieIds" :key="movieId" @click="router.push({ name: 'detail', params: { movieId } })">{{ t('wishlist.openMovie') }}</Button><Button variant="outline" :disabled="busy" @click="patch({ completed: !item.completed })">{{ t(item.completed ? 'wishlist.restore' : 'wishlist.complete') }}</Button><Button variant="outline" :disabled="busy || ['queued', 'running'].includes(item.enrichmentState)" @click="retry">{{ t('wishlist.retry') }}</Button></div>
