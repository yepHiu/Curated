	...
	for {
		...
		_3:
			...
			if *(*_VectorElementType)(unsafe.Pointer(bp + 40)) == int32(_SQLITE_VEC_ELEMENT_TYPE_INT8) {
				libsqlite3.Xsqlite3_str_appendf(tls, str, __ccgo_ts+1669, libc.VaList(bp+56, int32(*(*Ti8)(unsafe.Pointer(*(*uintptr)(unsafe.Pointer(bp + 8)) + uintptr(i1))))))
			} else {
				if *(*_VectorElementType)(unsafe.Pointer(bp + 40)) == int32(_SQLITE_VEC_ELEMENT_TYPE_BIT) {
					...
					libsqlite3.Xsqlite3_str_appendf(tls, str, __ccgo_ts+1669, libc.VaList(bp+56, int32(b)))
				}
			}
		}
		goto _1
...
func Xvec0_new_chunk(tls *libc.TLS, p uintptr, partitionKeyValues uintptr, chunk_rowid uintptr) (r int32) {
	...
	for {
		...
		libsqlite3.Xsqlite3_bind_int64(tls, *(*uintptr)(unsafe.Pointer(bp)), int32(1), rowid)
		libsqlite3.Xsqlite3_bind_zeroblob64(tls, *(*uintptr)(unsafe.Pointer(bp)), int32(2), uint64(Xvec0_metadata_chunk_size(tls, (*(*TVec0MetadataColumnDefinition)(unsafe.Pointer(p + 1600 + uintptr(metadata_column_idx)*24))).Fkind, (*Tvec0_vtab)(unsafe.Pointer(p)).Fchunk_size)))
		rc = libsqlite3.Xsqlite3_step(tls, *(*uintptr)(unsafe.Pointer(bp)))
		libsqlite3.Xsqlite3_finalize(tls, *(*uintptr)(unsafe.Pointer(bp)))
		if rc != int32(m_SQLITE_DONE) {
			return rc
		...
		*(*uintptr)(unsafe.Pointer(bp + 104)) = libc.UintptrFromInt32(0)
		...
		if rc == m_SQLITE_OK {
			...
			if (*(*TVectorColumnDefinition)(unsafe.Pointer(bp))).Fdimensions > uint64(m_SQLITE_VEC_VEC0_MAX_DIMENSIONS) {
				libsqlite3.Xsqlite3_free(tls, (*(*TVectorColumnDefinition)(unsafe.Pointer(bp))).Fname)
				*(*uintptr)(unsafe.Pointer(pzErr)) = libsqlite3.Xsqlite3_mprintf(tls, __ccgo_ts+5579, libc.VaList(bp+176, int64((*(*TVectorColumnDefinition)(unsafe.Pointer(bp))).Fdimensions), int32(m_SQLITE_VEC_VEC0_MAX_DIMENSIONS)))
				goto error
			}
	...
	*(*uintptr)(unsafe.Pointer(bp)) = libc.UintptrFromInt32(0)
	...
	for int32(m_true) != 0 {
		...
		if uint64(rowidsSize) != uint64((*Tvec0_vtab)(unsafe.Pointer(p)).Fchunk_size)*uint64(8) {
			...
			Xvtab_set_error(tls, p, __ccgo_ts+9653, libc.VaList(bp+168, uint64((*Tvec0_vtab)(unsafe.Pointer(p)).Fchunk_size)*uint64(8), rowidsSize))
			rc = int32(m_SQLITE_ERROR)
			goto cleanup
		}
